...
本项目的 BA 已接入 `MonocularVO.refine()`：由 `self.obs` 反查每条观测的像素坐标，构建 `obs=[(帧, 点, uv)]` 调用 `local_ba`；观测过多时确定性子采样（≤8000 条）控制时长。跑 `scripts/m1_run_vo.py --ba` 即可在 VO 跑完后自动重优化。
...
> **💡 直白讲解（一个反直觉、但极有价值的结果）**：在本项目实测里，BA 把
> **重投影误差从 0.444 px 打到 0.000 px** —— 说明 BA 的数学完全正确、收敛到位；
> 但 **ATE 在短序列上几乎不变（0.0529 → 0.0533 m）**。
>
> 原因有两点：① BA 优化的目标就是**重投影误差**，不是 ATE；② 短序列里 VO 本身漂移就很小，没多少"可修的漂移"。
> **对 ATE 的显著改善要在更长、漂移更大的序列上才明显——而那时 BA 也更慢**，
> 这正是为什么生产级系统（ORB-SLAM）要用**局部 / 增量 BA（只在关键帧窗口内做）**，而非全序列批量 BA。
> 这恰恰也是 M4（前馈 3D 模型）想"一次前向取代整条管线"时必须直面的代价之一。
