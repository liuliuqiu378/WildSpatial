...
> ## 🧭 初学者导读（5 分钟版 · 零基础先看这里）
>
> **这篇讲什么**：把 M0 的一堆几何零件**拼成一条会跑的管线**——喂进一段视频，吐出"相机是怎么移动的"（轨迹）+ 一个 3D 地图。
>
> **一句话理解这条管线**（类比"闭眼摸黑走路，一步步记住自己走到哪"）：
> 1. **找路标**（特征）：每帧挑一些容易认的点（窗角、桌角）。
> 2. **认到同一个路标**（匹配）：认出"上一帧这个角和这一帧这个角是同一个东西"。
> 3. **剔除认错的**（RANSAC）：把乱配的连对扔掉。
> 4. **算我移动了多少**（位姿）：由匹配点算相机在两帧间的旋转+平移。
> 5. **记住路标在哪**（三角化）：两条视线相交 → 得到 3D 坐标。
> 6. **下一帧靠已知路标定位**（PnP）：不用再从头算，直接用地图反推自己在哪。
> 7. **微调全部**（BA）：把"相机位置"和"路标位置"一起微调，让整条轨迹更自洽。
>
> **★ 本篇最大价值不是数字，而是"踩坑"**：文中记录了 3 个真实翻车（视差不足起不来、尺度检查顺序写错、位姿指数爆炸 RPE 到 49940m）。
> **"崩掉的那刻，才是真正理解的开始。"**
>
> **建议读法**：先看 §1 管线图 + §7 的三张结果图（轨迹/诊断/ATE 漂移）；
> 公式（八点法、E 分解、高斯-牛顿）第一遍可跳过。BA/回环这两节可以最后再看。
>
> **读完能回答**：为什么单目 VO 会"缓慢漂移"而不是突然崩？为什么手搓 VO 的 ATE（0.5m）比 ORB-SLAM（0.02m）差这么多？
...
> **🖼️ 结果图 1 · 轨迹对比**（运行 `scripts/m1_run_vo.py` 生成）：
...
> **🖼️ 结果图 · BA 前后轨迹对比**（运行 `scripts/m1_run_vo.py --ba` 生成）：
...
## 9. 🏭 实际应用：手搓 VO/SfM 能落到什么真活

本篇不是"为了手搓而手搓"——亲手搭一遍传统管线，是**看懂所有现代方法参照系**的必经之路，对应几类真活：

- **评审 / 选型前馈模型**：只有知道 VGGT 取代了"特征→匹配→RANSAC→三角化→PnP→BA"整条链，才能讲清"它强在哪、会在哪崩"（接 `M4`/`M5` 方法动物园）。
- **SLAM 调参与救火**：线上 VO 漂了，你能从"特征够不够 / 内点率高不高 / 护栏有没有"三处定位，而不是瞎调参（本篇 §2.4 护栏就是线上救火第一招）。
- **回环 / BA 集成**：知道纯 VO+BA 打不过带回环的 SLAM（§5 反直觉结论），才不会在量产里误用。

**面试 / 简历讲法（可直接背）**：
> "我手搓过单目 VO 并在 TUM fr1/desk 跑通（ATE 0.506 m），亲历三个真实翻车：视差 0.41° 起不来、尺度检查顺序写反、一次 PnP 失败没护栏导致 RPE 爆到 49940 m。所以我知道前端几何和'系统不崩'是两回事——这也是我理解 VGGT/ORB-SLAM 为什么这么设计的参照系。"

> 想看"崩在哪"的系统量化 → `M3`；想看前馈模型对比 → `M5`；想看手搓闭环怎么接控制 → `P1`/`P5`。
