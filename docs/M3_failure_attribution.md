...
> ## 🧭 初学者导读（5 分钟版 · 零基础先看这里）
>
> **这篇讲什么**：前面 M0/M1 在"干净数据"上跑通了，但真实世界会**下雨、起雾、变暗、抖动**。
> M3 系统回答：**什么样的坏条件会让系统崩、崩到什么程度、崩在哪一环**。
>
> **先记住 3 句话**：
> 1. **退化不是"变差一点"，而是"直接死掉"**：重运动模糊/暗光会让 VO **初始化失败、地图点=0**（硬失效），
>    而不是误差变大一点（软劣化）。分清这两类是工程上的生死线。
> 2. **最先断的是"匹配"，不是"找特征"**：运动模糊变重时，特征点还有 164 个，但**能配上对的从 169 掉到 6**
>    ——模糊摧毁的是"描述子能不能认出来"，不是"能不能看见点"。
> 3. **内点率是最灵敏的"预警信号"**：健康时 75~80%，崩之前会先跌到 11%——所以可以拿它做**运行时失效报警**（这直接喂给 M6）。
>
> **一个反直觉发现**：高斯噪声让**特征数反而变多**（1345→1938），但地图点却崩掉 78%。
> 原因：噪声造出一堆"假特征"——**看得见、配不上**。所以"特征数"是**误导性指标**，别只看它。
>
> **建议读法**：先看 §3 的「直白讲解」+ §4 的三张图（失效图谱 / 样本图 / 状态分布）+
> §6 的四条结论；"受控对照实验""severity 定义"这些方法细节可快速扫过。
>
> **读完能回答**：怎么给一个 SLAM 系统做"体检"？为什么说噪声和模糊是两种不同的死法？
...
本篇的"受控退化 + 逐环探针"不是学术游戏，而是**把"为什么崩"变成可观测、可预警的工程能力**，对应几类真活：

- **运行时健康监测**：用内点率 / 匹配数当预警（健康 75~80%，崩前跌到 11%）——比 ATE 早报警，直接喂给 `M6` 诊断切换。
- **测试基线**：M3 的 13 档失效图谱就是 `F10` 压力 / 退化场景库的原料——每条都能落成一条测试项（判据 = 内点率不破阈值 / ATE 不破限）。
- **给客户 / 监管讲风险**：能说清"哪种坏条件会硬失效、哪种只是软劣化"，而不是甩一个平均 ATE。

**面试 / 简历讲法（可直接背）**：
> "我不会只看最终 ATE——系统崩了 ATE 都没定义。我给管线每环装探针（特征 / 匹配 / 内点率 / 地图点），内点率从 80% 跌到 11% 时就报警，比轨迹误差早得多。噪声会让特征数涨 44% 但地图点崩 78%，所以'特征数'是骗人的指标——我用内点率做健康度。"

> 想看这套探针落成测试项 → `F10`；想看前馈模型在同套退化下表现 → `M4`/`M5`；想看预警后怎么兜底 → `M6`。
