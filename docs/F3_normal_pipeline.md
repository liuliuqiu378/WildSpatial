...
> **⚠️ 初学者常漏（F3 流水线篇）**：
> 1. **以为这条 Happy Path 在退化下也成立**：②③ 步靠"特征匹配"，纯旋转（无平移）、无纹理墙、强运动模糊会直接让它崩——这正是 `M3/M5` 存在的理由，别等上线才第一次见。
> 2. **忘了"回环"**：只靠逐帧累积（④⑤）走一圈回到原点会对不上（漂移）。要回环检测（重访校正）才能保地图一致，否则导航不可用（`F9 §9.3`）。
> 3. **以为有"全局尺度"**：纯单目 SfM 出的轨迹形状对、大小任意，必须外部锚定才变米制（同 F2 第 2 点，本项目 P4 用 LiDAR 定标）。
