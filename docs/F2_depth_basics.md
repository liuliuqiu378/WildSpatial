...
> **⚠️ 初学者常漏（F2 深度篇）**：
> 1. **以为单张照片能拿到深度**：投影丢了一维，近处小猫和远处老虎可投到同一像素——单目**先天无深度**，必须靠运动/双目/RGB-D/学习补（见 §1）。
> 2. **把"相对深度"当"绝对深度"**：学习式单目（Depth Anything / MiDaS / VGGT）输出是"谁近谁远"，**没有米制**。要公制必须锚定真尺度（LiDAR median / RGB-D / 已知尺寸物体），本项目 P4 靠 LiDAR 定标 ×72.433。
> 3. **忘了双目远处精度骤降**：深度误差 $\propto Z^2$，远处视差趋 0，双目只适合中近距（一般 <20–50 m，视场景）。
