...
`P5` 是本项目**唯一一条跑在真实驾驶数据上的「感知 → 规划 → 控制」全链路**（虽控制为运动学仿真、非真机，但接口契约与工程结构与量产一致）。它对应工业里这几类真活：

- **自动驾驶/AMR 导航栈（Nav2 同类）**：占据栅格 → inflation 膨胀 → A\* 规划 → (v,ω) 控制，正是 ROS2 Nav2、园区配送车、仓储 AGV 的标准导航链路。`P5` 的价值是让你**亲手走通这条链路并讲清每个环节的输入输出**，而不是只会调参。
- **动态避障（VO 式）**：用"估计移动目标速度 → 减速让行"做实时避障，对应真实系统的**速度障碍法（VO）/ 模型预测控制（MPC）**入门版——你能讲清"为什么不能等检测到障碍才刹，要提前预测"。
- **安全降级设计**：`P5` 隐式依赖 `P3/M6` 的"定位失效→降级"逻辑；量产系统必须有**故障树 + 降级模式**（见 `P6 §7` ODD/FMEA），这是"会跑 demo"和"能交付机器人"的分水岭。

**面试/简历讲法（可直接背）**：
> "我在同一份 KITTI 真实驾驶数据上，把建图接成了完整闭环：真实 LiDAR 累积占据栅格 → 按机器人半径+安全余量做 inflation → A\* 出路径 → 差速轮 (v,ω) 控制 → 对移动车辆做 VO 式减速让行。规划路径 8181 格、避障 18 步、控制 349 条指令，全程可回放。我主动说明控制是仿真级、未上真车——这正是项目'感知为主、控制为辅'的诚实定位。"

> 想看这条闭环在**新场景（无人船）怎么复用** → `P6`（把栅格语义换成可航水域即可）；想看**全栈怎么从硬件一路串到控制** → `F8`。
