...
- **验证闭环（下游）**：本篇的 13 退化 + 对照结论，可直接当 `F10 测试与验证` 的**压力/退化场景库**——每类退化都能落成一条测试项（判据=ATE 不破阈值/触发兜底）。

---

## 7. 🏭 实际应用：工业 SLAM 黑盒实跑能落到什么决策

本篇不是"为了跑 SLAM 而跑 SLAM"，而是用**工业级系统的实跑/对照**，把一个最常被问的工程问题讲清楚：**"我该在真实项目里用 ORB-SLAM3 / LIO-SAM，还是用你们的前馈模型 VGGT？"** 它直接对应这些真活：

- **选型决策（V&V 基线）**：量产项目落地前，先跑通一个成熟 SLAM（ORB-SLAM3）当**性能基线**，再决定自研/前馈模型能否替代。本篇 §4.1 的对照表就是这份"该不该换"的论据——例如"无纹理/夜间场景传统 VO 塌、VGGT 稳"→ 这类工况让 VGGT 当主定位或兜底。
- **工程踩坑预演**：§3 的 Eigen/Pangolin/尺度死穴清单，是每一个真上 SLAM 的项目都会踩的坑。你能提前讲出"为什么桥下丢星会尺度崩、为什么需要先装依赖再构建"——这是**学生 vs 干过活的人**的分界。
- **激光 vs 视觉主导的取舍**：LIO-SAM（激光主导）在退化场景比纯视觉稳，但贵且重；本篇帮你在 `F9` 选型四联表里补上"激光 SLAM"这一列的真实依据。

**面试/简历讲法（可直接背）**：
> "我实跑过 ORB-SLAM3 和 LIO-SAM 黑盒，和本项目自研的 VGGT/VO 做了 ATE 对照。结论：传统 SLAM 在有纹理场景达 0.0x m，但在无纹理/夜间会塌；前馈模型 VGGT 抗退化但 up-to-scale、需外部尺度源。所以我给的选型建议是'视觉为主+激光/IMU 兜底'，和我们 M6 诊断驱动切换一致——不是盲目追新模型，而是按工况分诊。"

> 想看"按工况分诊"的完整失效图谱 → `M5_stress_test`（方法动物园）；想看"兜底怎么接进闭环" → `M6` / `P3`。
