...
**📌 三种主流开放词汇检测器横向对比（选型）**：OWL-ViT 只是其中之一，真实落地常在这三个里挑：

| 检测器 | 一句话定位 | 最强 | 最弱 / 坑 | 何时选它 |
|---|---|---|---|---|
| **OWL-ViT** | 把文本编码进检测头，零样本出框（本项目实测） | 纯开放词汇、上手快、轻 | 幻觉重（本项目 `a book` 返回 20 框）、小目标弱 | 原型验证、当"发现未知"的探针 |
| **Grounding DINO** | 文本 + 图像跨模态融合，短语级接地 | 短语/属性查询更准（"红色的杯子"比 OWL 稳） | 仍需后处理（如 SAM 出掩码）、较重 | 要"指得准"的工业拣选/抓取 |
| **YOLO-World** | 把开放词汇塞进 YOLO 骨架 | **快、可端侧实时**（同 YOLO 血统） | 词汇表规模/细粒度弱于前两者 | 端侧/实时、类别大但可控 |

> **🎯 选型直觉**：要"发现没见过的"→ OWL-ViT（当探针）；要"指得准还能出掩码"→ Grounding DINO（+SAM）；要"端侧实时还开放"→ YOLO-World。**三者都逃不掉 §2.3 责任切分**——开放词汇结果只能当 2D 候选，3D 坐标和最终决策必须交给几何 + 规划器，否则安全场景会"指错物体"直接出事（见本项目 `a book` 幻觉实例）。
