...
> **✅ 状态（2026-09-15）**：**ROS2 Jazzy + Nav2 已安装并跑通**（免 sudo，robostack conda 路径，313 包）；
> 新增 **§4.15 机器人第一视角 3D 送货演示**（纯合成 FPV，零 GPU 权限依赖，含 6 类路况）；
> 新增 **§4.16 Gazebo 真实 RGB 相机 FPV**（方案 A 落地，需 `render` 组权限）。
...
> 📦 **举个实际例子（园区送货机器人）**：你写了一个"看到人就让路"的算法。用真实数据（P0/P1 那种回放）只能验证：
> "给定这段**已录下**的视频，你的算法有没有标出人"。但**没法验证**"机器人真碰到人会怎么动"——
> 因为真实数据里**没有**机器人去撞人的那一帧（真撞了就出事了）。换成仿真：你在 Gazebo 里放一个真人模型，
> 让机器人**真冲过去**，看它是停下了、还是撞上了、还是绕开了——**任意次重试、零风险**。
> 这就是"在线决策" vs "事后分析"的差别，也正是 §4.14 动态障碍 demo 在做的事。
...
### 2.1 🎯 怎么选：一个"场景 → 平台"决策清单（不空谈参数）

上面是按"层级"排的。落到**你手里这个具体任务**，按下面清单对号入座：

| 你的任务 | 直接用 | 为什么 | 本项目已有 / 将做 |
|---|---|---|---|
| **园区扫地 / 酒店送物 / 仓库 AGV** | **Gazebo + ROS2** | 通用移动底盘、室内世界好搭、Nav2 开箱即用 | ✅ §4 全套（costmap / 感知实验台 / FPV 送货） |
| **自动驾驶感知 / 决策** | **CARLA** | 现成城市、照片级相机/LiDAR、车流红绿灯 | ⚠️ 待接入（§6 下一步） |
| **机械臂抓取 / 移动操作** | **Gazebo + MoveIt 2** | 接触物理 + 抓取动力学，通用机器人仿真器 | ✅ §4.12 mobile manipulation |
| **室内 VLN / 第一视角导航** | **Habitat** | 真实扫描房屋、第一视角数据现成 | — |
| **轻量 RL / 控制原型** | **MuJoCo / PyBullet** | 几行代码起物理，不关心渲染 | — |

> **一句话决策**：**做"机器人怎么在物理世界里走/拿"→ Gazebo；做"车在城市里怎么开"→ CARLA。**
> 本项目选 Gazebo，因为主战场是**通用机器人感知→控制闭环**（园区配送这类），而 CARLA 是"自驾专供、只有车"。
...
#### 📦 实际例子：园区配送机器人的 costmap 是怎么"长"出来的

把上面这套抽象谱系落到**本项目 §4.14 / F8 的园区配送机器人**身上，看它每一层具体填什么：

| 层 | 内容来源 | 这一格写了什么（举例） |
|---|---|---|
| `static_layer`（离线先验） | 建图时扫好的墙/柱子 | "(3.2, 1.0) 处是墙，绝对不能走" |
| `obstacle_layer`（实时动态） | 当前 LiDAR 扫到的东西 | "正前方 0.8m 有个**刚才没有**的箱子（保洁刚放的）" |
| `inflation_layer`（膨胀） | 车体半径 + 安全余量 | 墙和箱子周围 0.3m 画一圈"靠近就扣分" |

**关键点（对应上面"分层融合"）**：保洁放的箱子**不在离线地图里**（static 层没有它），
但 `obstacle_layer` 这一秒把它写进去了——于是机器人**绕开它**，而**不用重新建图**。
这就是"静态导入 + 动态感知在同一个 costmap 里融合"的实物含义。

> 对照图：§4.6 的 [`costmap_concept.png`](../experiments/P2_tb3_nav2/figs/costmap_concept.png)
> 把这三层画得清清楚楚（左=原始 LiDAR 点、中=障碍+膨胀带、右=三层叠加）。
> P1 的 `occ` 栅格就是"砍掉 inflation 层"的简化版——先懂简化版，再看工业级三层，差异一目了然。
...
#### 📦 真实例子：我们怎么用"同源"给 VGGT 打分（而不用真机）

落到本项目 §4.9：把仿真世界调成不同光照/材质，喂给 **VGGT**（前馈 3D 模型），
用**仿真免费真值深度**打分（RMSE≈0.17m，和真实数据同量级）。

- **为什么可信**：因为仿真和真实**同源**（§3.6.1）——VGGT 在仿真里"看错"的方式，
  和它在真机里"看错"的方式是**同一类**（都是几何/成像问题）；
- **为什么比真机方便**：真机要重新采集+标深度，仿真**改个光照参数真值就免费更新**；
- **这就是 ② real2sim 的平民版**：我们没用 3DGS 重建真实场景，但用"同源模型 + 可控世界"
  拿到了**可迁移的结论**。

> 将来想做完整 real2sim：用 §4.10 的参数化世界思路，把**真实园区扫描**重建进 Gazebo，
> 就能在"和真机同构"的世界里无限测试——这正是 §3.6.3 ② 的字面落地。
...
## 3.7 🔴 感知-决策解耦：两阶段分解 + 兜底分层（核心认知，2026-09-15）

> **用户洞察（关键）**："难点是不是正好在现实场景的感知上……可以分两个阶段：
> **场景感知建模的好，问题就退化为走格子问题了，和模拟是一样的；建模的不好，就可能会有各种问题，需要去兜底。**"

**这个判断是机器人架构的主流思想（感知-决策解耦 / 抽象边界），且正好定义了本项目的难度分布。本节系统化。**

### 3.7.1 一句话：Stage 1 是难点，Stage 2 是"与来源无关的几何问题"

```
现实世界（杂乱、连续、噪声、无穷维）
        │
   ┌────┴──────────────────┐
   │  Stage 1：感知建模      │  现实 → 结构化世界模型
   │  (M7 检测/跟踪/建图)    │  occupancy grid / 物体列表 / costmap + 置信度
   └────┬──────────────────┘
        │  干净的结构化世界模型（格子 / 几何）
   ┌────┴──────────────────┐
   │  Stage 2：决策控制      │  只消费世界模型，不关心它怎么来的
   │  (规划/避障/控制)       │  → 本质是几何/走格子问题
   └───────────────────────┘
```

**关键**：Stage 2 **不 care 世界模型是 Gazebo 真值给的、还是 M7 从相机推理出来的**——
它只接收"结构化世界模型"这个**抽象接口**。所以：

- **建模好** → 世界模型干净 → Stage 2 拿到与仿真**同构**的输入 → 问题退化为走格子/几何避障，和模拟没区别；
- **建模差** → 世界模型有漏检/误检/错位 → Stage 2 在"错误的地图"上求最优 → 翻车。

### 3.7.2 两阶段的职责边界

| 阶段 | 输入 | 输出 | 难点 | 本项目对应 |
|---|---|---|---|---|
| **Stage 1 感知建模** | 原始传感（RGB/深度/激光） | 结构化世界模型 + 置信度 | **难**（现实噪声、长尾、遮挡） | **M7**（检测/跟踪/语义） |
| **Stage 2 决策控制** | 结构化世界模型 | 速度/轨迹指令 | 几何/图搜索，已被充分研究 | **M4/M5/M6**（空间/建图/融合推理） |

> 一旦 Stage 1 交付了干净的世界模型，Stage 2 就是 §3.5 讨论的"世界表示谱系"上的几何问题——
> 这正是 §4.15 / §4.16 的 FPV demo **只用真值/已知坐标就能完美跑通**的原因：它**完全活在 Stage 2**，
> 相当于"建模完美"的理想情形，先把决策-控制闭环验证掉。

### 3.7.3 为什么"建模好"就等同于仿真

- 仿真与真实共享底层模型（§3.6.1 同源），**区别在于仿真直接给你 Stage 1 的真值输出**。
- 真实系统若 Stage 1 足够好（M7 输出 ≈ 真值），那么 Stage 2 看到的输入和仿真**逐像素一致** →
  在仿真里验证的规划/避障策略可直接迁移（sim2real 合法性的另一面）。
- 反过来说：**本项目的 FPV demo 用真值当 Stage 1，等于在"假设 M7 完美"前提下验证 Stage 2**，
  这是合理的第一步（先跑通闭环，再单独攻感知）。

### 3.7.4 建模不好时的兜底：三层保险（由外到内）

现实里 Stage 1 永远不完美，所以要设计**"不信任模型"时的多层保险**：

| 层 | 名称 | 机制 | 依赖 | 作用 |
|---|---|---|---|---|
| ① | **不确定性感知** | costmap 带置信度；未知/低置信格子按"可能是障碍"处理 | Stage 1 输出 | 保守但不停车， optimality 下降 |
| ② | **反应式安全层** | 读**最原始传感**（深度近距/激光/bumper）做急停/速度阻尼 | 不依赖 Stage 1 语义结论 | **保命**：即使 Stage 1 完全错也能兜住 |
| ③ | **降级策略** | 感知置信度 < 阈值 → 减速 / 停车待确认 / 请求人工 | Stage 1 置信度 | 优雅失败：把"做错"变成"不做" |

**核心结论**：难点在 Stage 1，但 **Stage 2 的鲁棒性可独立保证**——
只要第 ② 层反应式安全兜住，建模差顶多"走得蠢/停住"，不会"撞上去"。
架构上应让 ② 层读最原始传感、跨过 Stage 1 的语义抽象，形成安全底线。

#### 📦 真实翻车案例：M7 在夜间漏检行人，三层兜底怎么接住

上面说"建模差就翻车、兜底接住"。下面用 **F8 的「园区夜间配送」场景**走一遍**建模失败的那一秒**，
看三层保险怎么逐层生效（这是 §3.7 最该被"看见"的一段）。

**背景**：夜间低光照，M7 检测器对远处行人的置信度掉到 0.3（低于 0.5 阈值）→ 行人**没被写进 costmap**。

| 时刻 | Stage 1（M7 感知） | Stage 2（规划/控制） | 兜底层 | 结果 |
|---|---|---|---|---|
| t=0.0s | 行人置信 0.3，**漏检** | costmap 里**没有**这个人，规划"直行" | — | 机器人照直走 |
| t=0.2s | 仍漏检 | 继续直行，离人 0.9m | ① 不确定性层：该格"未知"按保守处理，**略减速** | 慢了点，但**没停** |
| t=0.4s | 仍漏检 | 离人 0.6m，路径仍穿过 | ② **反应式安全**：读原始深度图，发现"正前方 0.6m 有东西、costmap 没标"→ **急停** | ✅ **停住，没撞上** |
| t=0.5s | 置信回升 0.6，**检出了** | costmap 补上这个人 → 重新规划绕行 | ③ 降级解除，恢复正常导航 | 绕开，继续送货 |

**这段说明三件事**：
1. **建模差 ≠ 撞车**：第 ② 层读的是**最原始的深度图/近距激光**，跨过了 M7 的语义结论——
   即使 M7 完全瞎了，机器人也不会撞上去（"保命"底线）。
2. **兜底有代价**：t=0.2s 那一下"略减速"就是 optimality 下降——但这是**优雅的代价**，不是灾难。
3. **这是仿真最容易复现的**：把世界调成"夜间低光"（§4.10 物理级退化 / §4.8 可控退化图库），
   就能**无限次重放**这段"漏检→兜底"——真实世界里你**不敢**故意造一个漏检去测。

> 对照图：§4.8 的 [`degradation_gallery.png`](../experiments/P2_gz_sense_degrade/figs/degradation_gallery.png)
> （低光使亮度 144→26，正是 M7 漏检的成因）；§4.14 的 [`ped_tracking.png`](../experiments/P2_dynamic_obstacle/figs/ped_tracking.png)
> （感知→决策→控制的心电图，这里反过来就是"感知失败→兜底"）。

**一句话**：难点永远在 Stage 1（感知），但 Stage 2（决策控制）的**鲁棒性可以独立保证**——
靠的就是"不信任模型"的多层兜底。这就是 §3.7 整套思想落进一个真实工况的样子。

### 3.7.5 与本项目落点 + 当前状态

- **模块映射**：M7 ↔ Stage 1；M4/M5/M6 ↔ Stage 2 在结构化模型上的推理（§3.5 谱系）。
- **当前**：FPV demo（§4.15 合成版 / §4.16 Gazebo 真实相机版）用真值/已知坐标填充 Stage 1 输出，
  验证了 Stage 2 闭环；感知实验台（§4.7–§4.13）在为 M4/M5/M6 供带真值数据。
- **待做（建议）**：
  1. 把 Stage 2 写成**与感知来源无关**的 `WorldModel` 接口（障碍列表 + 占据栅格 + 置信度）；
  2. 用 M7 检测器填充**同一个 `WorldModel`**，Stage 2 一行不改；
  3. 加 **反应式安全兜底**（读深度图近距急停 + 置信度接口），先在一致 Stage 2 上量化
     "真值感知 vs M7 感知"的差距——M7 漏检了、Stage 2 就怎么翻车、兜底层怎么接住。

### 3.8 🎬 把 §3.5 / §3.6 / §3.7 串成一个真实场景（防止"三节各说各话"）

这三节分别讲了"世界怎么表示""仿真和真实啥关系""感知-决策怎么解耦"——**但它们是同一趟任务的三面**。
用 **F8 的「园区夜间配送」**一句话串起来：

- **§3.5（表示）**：机器人脑子里只有一张 **costmap**（静态墙 + 动态箱子 + 膨胀带三层叠加）；
- **§3.6（同源）**：这张 costmap 在 Gazebo 里有**免费真值**，所以我们可以先在仿真里把决策跑通，再迁移真机（同一套几何/成像模型）；
- **§3.7（解耦）**：只要 M7 把行人写进 costmap（Stage 1 建模好），决策就是纯几何走格子；
  建模差了，**三层兜底**接住（就是上面那个夜间漏检案例）。

> 👉 想看这趟任务**完整逐秒发生**的样子（标定→定位→融合→WorldModel→规划→避障→控制→兜底→送达，
> 每步配真实图）？直接去 [`F8 全栈贯通`](F8_real_scenario_walkthrough.md)——
> 本文 §3 的抽象概念，在那里全部变成"看得见的一段旅程"。
...
> —— **静态图确实看不到"运动"**。本节给出 **GIF 动图**（495 帧俯视全景 @10fps）。

![动态障碍闭环动图（俯视全景）](../experiments/P2_dynamic_obstacle/figs/closed_loop.gif)

> **动图里能看到**（如同一场比赛的俯视回放）：
> - **蓝方块 + 蓝色朝向箭头** = 机器人，**箭头方向就是它头朝哪**，应当**指向紫色五角星**；
> - **紫色五角星 + 虚线连线** = **当前目标点**，虚线即"它要去哪"的直接证据；
> - **紫色点线八边形** = **巡逻路线**（8 个航点依次经过，半径 1.5m）；
> - **紫色虚线小圈** = **到达圈**（0.28m，进入即切换下一航点）；
> - **三个同心安全圈** = **黄(1.2m)预警减速 / 橙(0.85m)危险避让 / 红(0.5m)停止**（分层策略可视）；
> - **红圆 + 红色箭头** = 行人（在场地**两侧**往复穿行，箭头指其运动方向）；
> - **左上状态框** = `前往目标`绿 / `预警减速`黄 / `避让`红 / `停车`深红 / `绕行后回归目标`橙 **实时切换**；
> - **右上指标** = 时间 / v / 到目标距离 / 朝向误差 / 最近行人距离 / **安全区级别**。
>
> **一眼看懂"感知→决策→控制"**：机器人**朝紫星走** → 行人靠近进入橙圈 → 状态变红 →
> **朝向偏离紫星绕行**（朝向误差跳到 60°+）→ 行人远离 → 回归朝目标（误差回到 ~0°）→ 到达切换下一航点。

**⚠️ 修复一（机器人"跳来跳去"）**：初版动图里机器人**"跳来跳去、不知道在干什么"**——
根因是 `tb3_sandbox` 世界**墙间距仅 1.1m**（太窄），机器人一动就撞墙卡死。
**修复**：新建 `open_plaza_dynamic.sdf.xacro`（**开阔广场 + 四周外墙**）+ 改用**环线巡逻 + 状态机避让**，
机器人终于呈现**清晰的巡逻轨迹 + 绕行行为**。

**⚠️ 修复二（行人"跳来跳去"）**：用户接着反馈**"行人咋跳来跳去的？"**——实测坐实并比预想更严重：

| 现象 | 实测数据 | 根因 |
|---|---|---|
| 行人**撞墙后瞬移** | t=2.0s 时 y 从 4.50 **突跳到 1.78**（Δ=2.7m） | `mu=0` 零摩擦 + `VelocityControl` 恒推 → 撞墙穿透（tunnel）后被约束解算弹出 |
| 行人**最终卡死** | t=5s~13s 恒为 y=4.68（=5.0−0.5−0.22，贴在北墙） | 撞墙后动能耗尽，被引擎"粘"在墙上 |

**根因**：世界里的行人只被赋予**恒速**（`<initial_linear>0 ped_speed 0</initial_linear>`），
**没有任何边界/折返逻辑**，一路滑到 10×10m 场地外墙，撞墙穿透后产生瞬移，最后卡死在墙边。

**修复**：采集脚本对行人做**边界反射（boundary reflection）**——
按真值位置监控，`|y| > 3.0` 就向 `/model/pedestrian{1,2}/cmd_vel` 发**反向速度**，
使行人在场内 y∈[-3.0, 3.0] 平滑往复（离墙留 1.5m 余量，从根上不碰墙）：

```112:132:scripts/p2_dynamic_obstacle_gif.py
...
| 越界（\|y\|>4.9） | 常态发生 | **0 次** |
| 单步位移 | 瞬移 2.7m | **平滑（~0.35 m/s 恒定）** |
| 卡死 | t=5s 后完全静止 | **持续往复，从不静止** |
| 方向反转 | 无规律弹跳 | **规律折返** |

> **这就是"动图暴露静态图看不见的问题"的价值**：静态图上看不出行人已卡死在墙角。

**顺带澄清"目标点在动是不是在追踪？"**：**不是追踪**。目标点是**巡逻环线上"前瞻 1 秒"的航点**，
由时间函数 `patrol_target(t)` 生成（与机器人位置无关），所以紫星看起来"沿圆圈匀速转"。
本次新增**环线虚线**，让这一点在图上可直接读出。

**🔴 修复三（目标点形同虚设 —— 用户再次一针见血）**：用户追问
**"这里机器人的目的是什么，没有和五角星目标有什么关系呀，没有朝着五角星前进"**——**完全正确，这是范式性错误**。

**① 代码级根因**：旧版控制律用的是**环线解析切线**，全程**没有用到目标点**：

```python
cur = patrol_target(now); nxt = patrol_target(now + 0.5)
tangent = np.arctan2(nxt[1]-cur[1], nxt[0]-cur[0])   # ← 朝向来自时间函数
target_theta = tangent + off                          # ← 与 goal 无关！
```

而 `goal = patrol_target(now + 1.0)` 只是**同一时间函数的提前量**——它和机器人实际位置
**没有任何因果闭环**。所以：机器人不朝紫星走（朝数学切线）、紫星自己转圈（时间驱动）、
两者互相独立，目标点沦为装饰。
...
| 范式 | 目标点含义 | 控制律 | 旧版误用 |
|---|---|---|---|
| **轨迹跟踪** tracking | 参考轨迹上的一个点 | 跟切线 + 横向误差 | ← 旧版写的是这个 |
| **目标点导航** goal navigation | **实际要去的地方** | **`atan2(goal − robot)` 朝目标转** | ← 却画了目标点 |

**③ 修复（改为标准航点追逐 waypoint pursuit）**：

1. **固定航点表**（八边形环线 8 点），紫星 = **当前正在追的航点** `WAYPOINTS[wp_idx]`；
2. **朝向控制律** = `atan2(goal_y − y, goal_x − x)` —— **直接朝目标转**；
3. **到达判定**：距目标 < 0.28m → **切换下一航点**（紫星跳过去，因果明确）；
4. **避让**：在"朝目标"基础上叠加偏置 `off`，避完继续朝目标；
5. **图上新增**：航点连线（巡逻路线）+ **机器人→目标连线** + **机器人实际朝向箭头** + 到达圈虚线。

**④ 实测验证（核心命题：是否真的朝目标走）**：

| 指标 | 结果 | 说明 |
|---|---|---|
| **朝向误差 中位数** | **0.1°** | 巡航时几乎完美对准目标 |
| 朝向误差 <15° 占比 | 56.9% | 巡航帧对准 |
| 朝向误差 均值 | 26.3° | 被**避让帧拉高**（`off=±1.1rad=63°` 是故意偏转，自洽） |
| 到目标距离 | 周期收敛至 **0.28m** 后跳走 | 到达圈判定生效 |
| 目标点跳变次数 | **11 次** | 每次 = "到达并切换下一航点" |

> **巡航对准则 ~0°、避让偏离则 ~63°** —— 数字与设计自洽，证明逻辑正确，
> 而不是"看起来像在动"。

**⑤ 这次修复的深层价值**：**"能画出目标点"≠"真的在朝目标走"**。
如果只看截图或只看"跑通了"，这个范式错误会一直被掩盖；
是用户"**它到底在干什么**"的追问（行为层质疑）把它暴露出来 ——
这与修复二（动图暴露卡死）是同一条方法论：**可视化 + 质疑行为，才能发现逻辑错误**。

**🔴 修复四（避让质量差 —— 用户"避让不够优" + 关键教训：先修场景再修算法）**

用户反馈 **"行人进入其黄色虚线范围后的避让不够优呀"**。我连续调了三版**避障算法**，
但最小距离始终在 0.29–0.38m 打转（**均小于碰撞距离 0.44m**）——直到做**几何分析**才找到真因。

**① 量化诊断（算法层的真实症状）**

| 问题 | 实测 | 严重性 |
|---|---|---|
| 贴脸仍全速走 | 距离 <0.7m 时 **v 恒为 0.15 m/s（133/133 帧）** | 速度与距离**完全无关** |
| 实际碰撞 | 最小距离 **0.368m < 0.44m**（0.22+0.22） | **已穿透** |
| 大部分时间在挣扎 | **39.1%** 帧在危险区 | 避让效率低 |
| 只有两档速度 | CRUISE 0.28 / AVOID 0.15 | 无连续调节 |

**② 三版算法演进（逐步改善，但都没解决根本问题）**

| 版本 | 策略 | 最小距离 | 碰撞? | 危险区时间 |
|---|---|---|---|---|
| v1 | **开关式**：进圈固定 `off=1.1, v=0.15` | 0.368m | ❌ | 39.1% |
| v2 | **比例式**：速度/偏转随距离**连续**变化 | 0.293m | ❌ | 11.1% |
| v3 | **预测式**：用行人未来位置触发 + 停车侧移让路 | 0.381m | ❌ | 3.6% |
| **v4** | **场景修正**（见下）+ 保留 v2/v3 算法 | **0.782m** | **✅** | 3.6% |

> **v2 的关键发现**：v2 后最小距离反而降到 0.293m，追踪该时刻发现
> **机器人已静止（v=0 持续 12 帧），dmin 却从 0.41 降到 0.29** ——
> 说明是**行人在主动走向静止的机器人**。**"停车"是被动策略**，无法避免被撞。

**③ 🔴 真因：几何上无解（场景设计问题，不是算法问题）**

```
机器人巡逻环线：x² + y² = 1.5²
行人路径 x = 0   → 与环线交于 y = ±1.50   ← 必经冲突点
行人路径 x = 1.2 → 与环线交于 y = ±0.90   ← 必经冲突点
```

**4 个必经冲突点**：机器人必须穿过人流才能巡逻，**无论算法多好都无法完全避免贴脸**。
—— 我此前连调三版算法，是在**错误的地方使劲**。

**④ 修复：分离"行人走廊"与"机器人车道"**

| | 旧 | 新 |
|---|---|---|
| 行人站位 | x=0 / x=1.2（**纵切环线中央**） | **x=±2.6（环线外侧）** |
| 冲突点 | **4 个（）必经** | **0 个（可控可避）** |
| 机器人 | 环线半径 1.5m | 不变，但**有空间绕到行人身后** |

**⑤ 最终实测（v4）**

| 指标 | 结果 | 评价 |
|---|---|---|
| **最小距离** | **0.782m** | **> 0.44m，安全余量 1.8 倍 ✅** |
| 停止圈时间 | **0.0%** | 再也不用急停 |
| 安全巡航时间 | **90.1%** | 从容行驶 |
| 速度-距离 | **0.130→0.197→0.263→0.278 严格单调** | 教科书式比例控制 ✅ |

**⑥ 这次的核心教训（可复用）**：

> **"避障不够优"未必是算法问题，先检查场景是否几何可避。**
> 正确顺序是 **① 保证几何可避 → ② 再优化避障算法**。
> 若场景本身"必经冲突"，任何算法都只能减小接近程度，无法避免碰撞 —— 这时调参是无底洞。
> **诊断手段**：把机器人路径与障碍路径**画在同一坐标系求交点**，若交点在机器人必经区 → 场景问题。

**⑦ 保留的算法改进（v2/v3 成果，本身正确）**：
- **比例减速** `v = v_max·(d−STOP)/(WARN−STOP)`（连续，越近越慢）；
- **比例转向** 偏转随距离连续增大（5°~75°）；
- **速度预测** 用行人未来位置（前瞻 0.8s）触发，**提前绕行**（而非贴脸反应）；
- **分层安全区可视化**：黄(1.2m)预警 / 橙(0.85m)危险 / 红(0.5m)停止。
...
- ⚠️ **`VelocityControl` 的指令 topic 名**（实测确认，易猜错）：
  `/model/<model_name>/cmd_vel`（另有 `/model/<name>/link/<link>/cmd_vel` 变体）——
  用 `gz topic -l` 可列出验证。踩坑：一度以为是 `<model>/cmd_vel` 或需 ROS bridge 转发。
- ⚠️ **`VelocityControl` + `mu=0` + 边界的陷阱（关键）**：
  恒速 + 零摩擦的行人**不会自己折返**，撞到墙会**穿透（tunnel）并瞬移**，随后卡死在墙边。
  **必须**在外部（脚本/插件）做**边界反射**，或改用 `TrajectoryFollower` 折线路径。
  推荐修复：按真值位置监控、越界即向 `cmd_vel` 发反向速度（本次采用，见 §4.14.3b 修复二）。
- 行人现在**场内往复穿行**（y∈±3.0m，速度 0.35 m/s），机器人始终有稳定的避让对象。
...
## 4.15 🎬 机器人第一视角 3D 送货演示（固定目标 + 六类路况）

> **用户要求**："保留当前这个（俯视版）作为基础版本，能不能再开发一个复杂情况的——
> **机器人视角、3D 画面、去固定地方送货、路上出现各种情况**？"

这一节是俯视版 `P2_dynamic_obstacle` 的**升级视角**：从"看机器人怎么动"变成
"**机器人自己看到什么、怎么反应**"，并且给机器人一个明确的任务——**把货送到固定地点**。

### 4.15.1 任务与 6 类路况

机器人从起点 `(0,0)` 出发，沿固定路线前往**目标建筑 `(0,10)`**；途中依次遇到：

| 编号 | 路况 | 机器人反应 | 教学点 |
|---|---|---|---|
| ① 静态障碍 | 路边停着快递推车 | 小幅减速 + 绕行 | 固定障碍的横向偏移 |
| ② 横向行人 | 行人横穿送货通道 | **停车让行** | 安全优先、让行规则 |
| ③ 对向行人 | 窄通道里迎面走来的人 | **靠右礼让 + 减速错车** | 窄空间会车规则 |
| ④ 视觉盲区 | 拐角被墙挡住视线 | **拐角前减速** | 感知受限时降速 |
| ⑤ 窄通道 | 两面墙夹道 | 居中精准通行 | 横向控制精度 |
| ⑥ 送达点 | 终点建筑 | 减速停靠在正面 | 任务完成、最终定位 |

### 4.15.2 实现方案：纯合成 3D FPV（方案 B）

**为什么不直接用 Gazebo 相机？**

本机 `--headless-rendering` 需要访问 `/dev/dri/renderD128`，
当前用户**不在 `render` 组**，Gazebo RGB 相机输出灰屏（已在 `scripts/_probe_fpv.py` 实测确认）。

**方案 A**：`sudo usermod -aG render $USER` 后重新登录 → 真 3D 渲染，但**需要环境变更 + 重新登录**。
**方案 B**：自研轻量软件渲染器，零 GPU 依赖，**可立即运行**——本次先用 B，让演示落地。

渲染管线：

```
机器人位姿 (x, y, θ) + 相机高度 h=0.55m
        ↓
针孔相机：水平视场 75°，focal = (W/2)/tan(37.5°)
        ↓
世界 3D 点 (X,Y,Z) → 相机右/上/前坐标 → 像素 (u,v)
        ↓
画家算法：按深度从远到近绘制
        ↓
...
```140:157:scripts/p2_delivery_fpv.py
# 针孔相机投影（世界 → 像素）
...
def project(P, C, right, up, fwd):
    ...
    xc = float(d @ right); yc = float(d @ up); zc = float(d @ fwd)
    if zc <= NEAR: return None
    u = CX + F * xc / zc
    v = CY + F * yc / zc
    return (u, v, zc)
```

效果与真实相机一致：近大远小、地平线汇聚、墙体**真实遮挡**后面的物体。

### 4.15.3 关键帧与动图

![六类路况关键帧](../experiments/P2_delivery_fpv/figs/keyframes.png)

![第一视角送货动图](../experiments/P2_delivery_fpv/figs/delivery_fpv.gif)

> - 主画面 = 机器人眼睛看到的 3D 世界；
> - 右上角**小地图** = 全局俯视（蓝方块=机器人，红点=行人，紫星=送达点）；
> - 左上角 = **当前工况**（红底白字实时切换）；
> - 左下角 = 速度 / 到送达点距离 / 时间。

### 4.15.4 与俯视基础版的关系

| | 俯视版 `P2_dynamic_obstacle` | 本 FPV 版 `P2_delivery_fpv` |
|---|---|---|
| 视角 | 上帝视角 | **机器人第一人称** |
| 3D 感 | 平面动图 | **透视 3D 画面** |
| 任务 | 沿环线巡逻 | **固定地点送货** |
| 工况数 | 动态行人避让 | **静态/动态/盲区/窄道/送达** 6 类 |
| 渲染方式 | Gazebo 真值 + matplotlib 俯视 | **自研针孔投影 + 画家算法** |

### 4.15.5 诚实边界

- **画面是合成渲染**，不是真实相机/Gazebo 相机输出；但透视、遮挡、尺度正确。
- 机器人沿**固定航点表**前进，尚未接入在线 SLAM/Nav2；本 demo 聚焦"第一视角"与"多工况反应"。
- 行人与障碍位置由脚本给定，真实系统需用 M7 检测 + 跟踪得到。
- 方案 A（Gazebo 真相机 FPV）已补充在 §4.16，需 `render` 组权限。

运行方式：

```bash
python scripts/p2_delivery_fpv.py
# 产出：experiments/P2_delivery_fpv/figs/{delivery_fpv.gif, delivery_fpv.mp4, keyframes.png}
```

---

## 4.16 🎥 Gazebo 真实 RGB 相机 FPV（方案 A）

> 在用户执行 `sudo usermod -aG render hmn-cjy` 后，本机 Gazebo `--headless-rendering`
> 获得 `/dev/dri/renderD128` 访问权限，于是把 §4.15 的纯合成 FPV 升级为
> **Gazebo 真实相机输出**。

### 4.16.1 与纯合成版的区别

| | 纯合成版 `p2_delivery_fpv.py` | Gazebo 真实相机版 `p2_delivery_fpv_gazebo.py` |
|---|---|---|
| 渲染 | 自研针孔投影 + 画家算法（CPU） | **Gazebo + Ogre2 + EGL headless（GPU）** |
| 3D 真实感 | 几何正确、风格化 | **真实光照/材质/阴影** |
| GPU 权限 | 无依赖 | 需 `render` 组 |
| 场景复杂度 | 6 类路况、自定义世界 | 复用 `open_plaza_dynamic` 广场 + 柱子/行人 |
| 工况标签 | 6 类 | 巡航 / ①静态障碍 / ②动态行人 / ⑥送达点 |

### 4.16.2 运行方式

```bash
# 当前会话未重新登录时，用 sg render 带组运行
sg render -c 'python scripts/p2_delivery_fpv_gazebo.py'

# 已重新登录且 id 显示 render 组，可直接运行
...
experiments/P2_delivery_fpv_gazebo/
    figs/delivery_fpv_gazebo.gif
    figs/delivery_fpv_gazebo.mp4
    figs/keyframes.png
    metrics.json
```
...
1. **`/clock` 仿真时间积分**：headless rendering 负载高，Gazebo 仿真时间比墙上时间慢。
   机器人与行人都按 `/clock` 推进，保证 HUD/小地图与相机画面同步。
2. **行人位置解析估计**：不依赖 `/world/default/pose/info` 的更新稳定性，
   而是按世界 xacro 中 `VelocityControl` 的初速度与边界直接计算。
3. **HUD 叠加**：用 PIL 在真实 RGB 帧上绘制场景标签、速度、到目标距离、小地图。

![Gazebo FPV 关键帧](../experiments/P2_delivery_fpv_gazebo/figs/keyframes.png)

### 4.16.4 诚实边界

- 仍是 **Gazebo 仿真相机**，不是真实物理机器人相机。
- 机器人控制为固定航点表 + 比例控制，未接入 Nav2/SLAM。
- 行人位置按已知运动模型估计，真实系统应由 M7 检测跟踪提供。
...
本篇不是"教装 Gazebo"，而是讲清**仿真在工业界的两大真活**——它既是带真值的数据工厂，又是闭环验证台：

- **感知实验台（V&V 资产）**：Gazebo 同帧输出带真值 RGB-D / 点云 / 位姿，且能随手改光照 / 雾 / 噪声——`M4` VGGT 评测、`M5` 失效图谱、`M6` 融合都在可控世界里跑（比合成退化更真、比真实数据更可控）。这直接对接 `F10` 的"仿真 SIL 打底"。
- **安全闭环复现**：真实世界不敢故意造"漏检撞人"去测，仿真里放个行人模型让机器人真冲过去，看它停没停——`§3.7.4` 夜间漏检 → 三层兜底接住，正是 `F10` 验收用例的源头。
- **sim2real 桥梁**：同源模型（几何 / 物理 / 成像）让仿真结论可迁移真机（§3.6）；real2sim（3DGS 重建真实场景进仿真）是前沿方向。

**面试 / 简历讲法（可直接背）**：
> "我用 Gazebo 当'带真值的数据工厂 + 闭环验证台'：M4 VGGT 在可控世界打分（RMSE 0.17 m，和真机同量级），M5 失效图谱在物理级退化里复现，动态避障 demo 735 步闭环、遇人减速让行。关键是'建模差靠三层兜底接住'这种长尾，仿真里能无限次重放、零风险——这正是量产迭代的标准方式。"

> 想看仿真怎么变测试项 → `F10`；想看真实数据闭环 → `P4`/`P5`；想看"解构成表示"的方法论 → `P6`。
