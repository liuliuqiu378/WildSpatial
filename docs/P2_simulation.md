...
> **动图里能看到**（如同一场比赛的俯视回放）：
> - **蓝方块** = 机器人，沿**蓝色环线轨迹**巡逻（不是原地打转！）；
> - **紫星 + 虚线** = 当前目标点（"它要去哪"）；
> - **橙色虚线圈** = 危险半径（1.2m，跟着机器人）；
> - **红圆** = 行人（在移动）；
> - **左上状态框** = `巡航（去目标）`绿 / `避让（绕开行人）`红 / `绕行中`橙 **实时切换**；
> - **右上指标** = 时间 / 前进速度 v / 转向速度 ω / 最近行人距离。
>
> **一眼看懂"感知→决策→控制"**：行人靠近 → 进入橙色圈 → 状态变红 → **机器人偏转绕行** → 行人远离 → 回归环线。

**⚠️ 一个重要修复（用户反馈驱动）**：初版动图里机器人**"跳来跳去、不知道在干什么"**——
