...
> 当前阶段：**教程体系（基础篇 F0–F13 / 模块 M0–M9 / 实战 P0–P6 / 总纲 00_* / 索引）与实验骨架已全部闭环，进入「清障 → 打磨 → 补差异化主场 → 真实数据验证」阶段**。
...
> **P2 仿真平台新增「机器人第一视角 3D 送货演示」**（固定地点 + 6 类路况，纯合成 FPV 零 GPU 依赖）→ `docs/P2_simulation.md §4.15` + `experiments/P2_delivery_fpv/`。
> **P2 仿真平台新增「Gazebo 真实 RGB 相机 FPV」**（用户加入 `render` 组后方案 A 落地，TB3 在真实 Gazebo 广场中送货，触发 ①/②/⑥ 三类工况）→ `docs/P2_simulation.md §4.16` + `experiments/P2_delivery_fpv_gazebo/`。
> **P3 面试旗舰项目已落地**：把「恶劣环境鲁棒感知 + 诊断驱动融合兜底 + 感知→规划→控制闭环」收口成**一个可讲解的完整落地项目**（所有模块 F/M/P0–P2 的收敛点）。可跑脚本 `scripts/p3_flagship_delivery.py` 在 TUM fr1/desk（100 帧）上实测：自研 VO 弱光/运动模糊**塌缩（路径长=0）**，**VGGT 三档 ATE 0.020–0.030m 稳压** → 融合兜底切 VGGT → 托底轨迹驱动占据栅格+A\* 30 步+差速控制 29 条+速度障碍法避障 5 步；配 `docs/P3_field_project.md` + `experiments/P3_flagship_delivery/`（4 图+metrics+README）。另给 `methods/vggt.py`/`colmap.py` 补充完整 `T_cw` 位姿输出（闭环建图需要）。
> **P4 真实驾驶动态建图已落地**：把「真实多传感器数据当实时输入流」跑通【真实 RGB → VGGT 视觉定位 → 真实 LiDAR 尺度定标 → 逐帧 BEV 增量建图 → **移动目标检测标红** → 第一视角+俯视全景双视角视频回放】。数据用**已在磁盘**的 KITTI depth_completion 真实街道驾驶（drive 0023，30 帧连续 ≈18s，零下载、非仿真非合成退化）：VGGT 一次前向出 30 帧 `T_cw`（up-to-scale），真实 LiDAR 深度 median 比定标 ×72.433 → 轨迹 **125.16 m**（≈7 m/s 合理）；每帧 1.7–2.0 万真实激光点、累计 **56.5 万点**累积成 0.4m 栅格 BEV（≈150×190m，街道走廊/两侧停车/行道树可辨）；**移动目标检测**（前序帧世界点=静态地图，KD-tree 3D 邻近匹配半径 1.2+0.03·Z m + 离地约束）30 帧共检出 **1 743 个移动点**，双视角标红。配 `docs/P4_real_driving.md` + `experiments/P4_real_driving/`（双视角 GIF+MP4+米制终图+6 关键帧+metrics+README）。诚实边界：基准无逐帧真值位姿→不报 ATE（构建型 demo）；轨迹来自视觉、尺度来自激光；移动目标检测为启发式（演示级，非检测器 benchmark）。**多场景 demo**：同管线另跑 0002(71.87m/22.6万点/1187红) · 0013(152.62m/40.4万/**4771红**) · 0095(155.52m/55.3万/2281) → `experiments/P4_real_driving/drives/<drive>/`。
> **F9 方法选型与工程清单已落地**（`docs/F9_perception_methods.md`，354 行）：补齐此前最大的讲解缺口——**"该不该用这个方法、用得起吗"**。单目（特征点 SfM/VO · COLMAP · 学习式前端 · 单目深度 · VGGT）、双目（SGBM · 学习立体 · MVS）、2D 检测/分割/开放词汇、3D 检测（PointPillars/SECOND/CenterPoint/BEVFusion）、LiDAR（ICP/GICP/NDT · LiDAR SLAM · 地面分割/聚类/3D 语义分割）、场景表示（点云/网格/体素/TSDF/占据/BEV/NeRF/3DGS）、融合（标定/时间同步/前中后架构）逐族给出**四联表**：一句话定位 + 精度量级 + 算力·显存 + 需要的数据量，并钉本项目实测数字（M5 ATE、M10 KITTI、P4 定标 ×72.433、M8 端侧）。另补 §7 训练**数据量·类别数·长尾·未见物体 OOD**、§8 算力设备表、§9 **初学者想不到但上真机就撞上**的工程清单（时钟同步/标定漂移/运动畸变/TF 树/标注成本/动态污染建图/回环/兜底降级/评测诚实/功能安全）。导航已联动 INDEX（F0–F9、覆盖表、路径 B/C/D、覆盖状态）+ README（文档地图 + §6.6 选型速查）+ ROADMAP（全局地图 + 最后一公里）。
> **M5 工业 SLAM 对照补篇已落地**（`docs/M5_industrial_slam.md`）：把 ORB-SLAM3（单目/双目/RGB-D/VI）当黑盒在 TUM fr1/desk 实跑的**工程踩坑清单 + 与 VGGT/方法动物园结论对照**，回扣三条核心结论（ORB-SLAM3=工业级版自研VO+回环v1；在线增量vs离线全局比经典vs学习更本质；单目尺度死穴+外部尺度源思路同源）。方法动物园从 4 方法扩展到「含工业 SLAM 的 5 方法对照」。LIO-SAM 留待有 LiDAR 真值流时补。本机实跑 ATE 为领域经验量级，未冒充项目实测（跑通后回填 `experiments/M5_orb_slam3/`）。
> **P5 真实驾驶端到端闭环已落地**（`docs/P5_real_driving_closed_loop.md` + `scripts/p5_real_driving_closed_loop.py` + `experiments/P5_real_driving_closed_loop/`）：补齐"**有真实感知但没接闭环、有闭环但只用室内/仿真数据**"的结构性缺口。**复用 P4 前端**（真实 RGB+LiDAR → VGGT 定位 → 定标 → 世界点云 → 移动目标），接 **① 2.5D 占据栅格（剔除移动目标格）→ ② inflation 膨胀（机器人半径 0.5m + 余量 0.3m）→ ③ A\* 规划 → ④ 差速轮 (v,ω) 控制（含限幅）→ ⑤ 用真实动态目标做 VO 式避障**。实测（drive 0023，30 帧）：栅格 487×369@0.4m，障碍格 5038，**剔除移动栅格 8181**（不剔除→地图留幽灵），**膨胀新增 5116 不可通行格**，A\* **492 步 / 211.84m**，控制 **491 条**（v_max 0.8m/s，ω_max 1.5rad/s），**避让 34 步**。产物：5 张图（占据/膨胀对比/规划/控制/避障）+ 闭环 GIF + metrics + README。诚实边界：自车运动为运动学仿真；地图来自视觉估计（无真值位姿）；避障为反应式 VO 简化版；**未做 ego-footprint clearing**（自车足迹清除，量产必备，已在文档标注为本 demo 差距）。
> **P6 新场景从零起步方法论已落地**（`docs/P6_new_scenario_perception.md`）：把「感知为主、控制为辅」策略**泛化成可复用 6 步脚手架**，用**无人船（USV）自主驾驶**当贯穿例子，回答「新场景什么都不具备、做感知给控制当输入，从哪下手」这一面试高频题。6 步 = ①写**控制契约**反推感知输出 → ②场景拆子任务 → ③工具箱选型（查 F9 四联表，复用 VGGT/OWL-ViT/M6/M8/P5）→ ④数据三路径（公开水域集 MODD/USVInland + 跨域迁移 KITTI + sim2real Gazebo）→ ⑤复用 P5 管线改写水域版（脚本骨架 `p6_usv_perception.py` 伪代码）→ ⑥量化+诚实边界+失效兜底。**§6 完整模拟演练已补**：把脚手架全假设填充成「可交付设计样例」——场景/任务(500m 码头A→B)、硬件清单(IMX477+VLP-16+F9P+Orin Nano 模拟)、控制契约(数值精度)、子任务→算法选型(每项的参数+假设精度)、数据假设链(MODD/GNSS伪真值/KITTI迁移)、端到端管线(900帧/47.3m轨迹/12船8浮标3桥墩/350步A*/349控制/18避让/12%兜底)、模拟 metrics.json、M8 端侧预算(INT8 VGGT 80ms+OWL 90ms→4.8Hz)。**全部标【模拟】非实测**，不破坏项目诚实边界。**§7 ODD+FMEA 安全概念模板 + §8 出海采集/真值载荷/标注规范 checklist 已补**（工业证据：ODD 表/安全概念/OSS/FMEA 6 条/COLREGs+船级社钩子；出海 checklist+RTK/IMU/AIS 真值载荷+标注长尾治理/回归集），均为流程模板非实测，收口「补生产级证据」缺口。**§6.10 可复现模拟脚本已落地**（`scripts/p6_usv_sim.py`，纯 numpy+matplotlib 零下载）：把 §6 假设画成真图——`experiments/P6_usv_sim/figs/usv_occupancy_plan.png`（600×400@0.4m 航道栅格：岸/浅滩/桥墩/浮标/反光区+A\*路径+2动船）+ `usv_glare_ate.png`（ATE vs 反光曲线，M6 兜底触发点 ≈0.55 可视化）+ metrics.json（与 P5 格式对齐）。脚本自跑实测：A\* 561 步/240.6m、避障 7 步、膨胀 54573 格、路径经过反光格=0（规划层天然规避感知退化区）；另已 web-search 真实水域数据集硬数字（MaSTr1325 1325 张真 USV / SeaDronesSee ~3000 张 / USVInland 多传感器内河 / MODD2）写进对话认知。
> **F10 测试与验证方法论已落地**（`docs/F10_testing_verification.md`）：补齐项目"重算法建模、弱交付验证"的短板。用 ISO 26262(故障安全) + ISO 21448 SOTIF(2022,功能不足安全) + PEGASUS 场景法 + Wachenfeld&Winner 66 亿公里里程悖论 锚定工业实践：① **测什么**——从 ODD 边界(P6§7.1)/FMEA 失效项(P6§7.3)/需求条目 三条线反推测试项，每个写"被测对象/前置/输入/判据"四要素；② **怎么测**——单元→集成→系统→验收四层 + 仿真 SIL/MIL 打底、HIL/VIL 实测收口（金字塔 90%仿真+10%实测）；③ **测多久**——纯里程法不可行→场景覆盖率替代里程数（场景库规模/边界覆盖/回归集通过率/OOD触发率/无碰撞率）；④ **验收准则**——量化 pass/fail + SOTIF 正风险平衡(系统风险低于人类驾驶同级任务)。给 P5 一套可直接抄的测试计划表（T1–T6）。与 P6§7(ODD/FMEA 即测试项来源)、M5(退化场景库)、F9(工程清单) 双向引用；导航联动 INDEX(§二加 F10 行 + 路径 B/C/D 加 F10 + §五收口)。
> **P4 补全三块**：(A) §⑧ 诚实边界新增第 7 条"**不含自动驾驶栈后半段**"（明确 P4 只有感知→建图，无规划/控制/避障，且 BEV 不能直接导航）+ 第 8 条"硬件匿名"；(B) 新增 **§⑨ 硬件对应**（KITTI 实车 PointGrey Flea2 相机 + **Velodyne HDL-64E 64 线** + OXTS GPS/IMU、为什么选 64 线、为什么选 KITTI、算力需求与端侧可行性），与 `F7 硬件扫盲` **双向交叉引用**；(C) 新增 **§⑩ 参数与规则设置**（BEV 0.4m / LiDAR 80m / cam_h 1.65m / 离地阈值 0.5m / 匹配半径 1.2+0.03Z **每个阈值的取值理由 + 调参方向** + 现有失效判据/降级规则表 + 与 P3 量化判据的差距说明）。
> **F11 建图实操全链路已落地**（`docs/F11_mapping_in_practice.md`）：补"初学者建图一站式"缺口（用户明确问：硬件/算法/格式/体积/存哪/怎么用/例子）。把散在 `F1`(表示)/`F3`(算法)/`F7`(硬件) 的零件串成全链路，并新补教程此前缺失的：① **5 种表示体积量级估算**（栅格@5cm≈4MB、100万点云≈16MB、TSDF@2cm房间≈75MB、mesh 84.8万顶点≈20MB、BEV≈百KB，明确标的"经验估算非实测"）；② **存哪/坐标帧(sensor→robot→map)/地图版本与多程拼接**；③ **园区扫地机器人完整例子**（硬件→跑→.pgm≈16MB→存磁盘→A\*用）；④ **新手想不到的 8 个坑**（外参漂移/动态物进静态图/回环失败/GPS多径/尺度锚错/帧混用/长期不一致/体积爆炸）。导航联动 INDEX(§二加 F11 行 + 路径 B/C/D 加 F11 + §五收口)、QUICKSTART(基础篇扩 F0–F11)、PROGRESS 顶部体系。
> **F12/F13 已就位**（`docs/F12_robot_taxonomy.md` + `docs/F13_metrics_calibration.md`）：① **F12 机器人任务分类**——按"环境先验（已知结构化 / 半已知 / 未知开放 / GNSS 拒止 / 极端）"而非"固定 vs 移动"重新划类，配 `f12_landscape.png` 地形气泡图，并给每类的架构 / 难度 / 与本项目映射；② **F13 指标速查 + 标定实操**——一张"你想证明啥 → 该看哪个指标"决策表（贴本项目真实量级）+ 三种标定（相机内参 Zhang / 相机-激光外参 T_cl / 时间同步 PTP）+ "1° 外参误差 @10m ≈ 17cm" + 调试铁律（先时钟 → 再外参 → 再算法）。同步给 `M1`/`M3`/`F9`/`F8`/`P2`/`P6` 补「🏭 实际应用」小节，把"理论-工具-真机落地"打通。导航联动 INDEX(§二加 F12/F13 行 + 路径 B/C/D 扩 F0–F13 + §四图索引 + §五收口)、QUICKSTART(基础篇扩 F0–F13)。
...
### 1.1.1 🟢 整体策略定位（一句话收口）：**感知为主、控制为辅**

> **用户 2026-09-17 明确**：整体想做「感知控制」，但**重点在感知、弱化控制**——
> 因为控制受硬件限制（无真车/真底盘），而**感知对硬件需求更低、可纯靠真实公开数据 + 本地算力推进**。
> 据此把项目重心与对外叙事统一为：

- **感知层（主场，重点投入）**：方法选型（F9）/ 失效图谱（M3·M5）/ 诊断驱动融合兜底（M6）/
  真实数据验证（D1 水下夜间 · P4 真实驾驶 · 工业 SLAM 对照 M5_industrial）/ 端侧（M8）/ 开放词汇语义（M7）。
- **控制层（辅，接口打通即可）**：`F6`/`P1`/`P5` 的定位 = **「感知→控制接口已打通、失效兜底设计已闭环，
  但控制指令为运动学仿真、无真机验证」**（硬件不允许）。对外讲主动把控制定义为
  「设计已闭环、待硬件验证」，**不冒充真车闭环**，避免被 reviewer 抓把柄。
- **对外一句话**：「我是**恶劣环境鲁棒感知**方向的工程师；感知→规划→控制闭环我打通过（运动学仿真级），
  真机控制待硬件，但感知在真实公开数据上定量跑通过。」
...
| **P3** | **面试旗舰项目（收敛点）** ⭐ | 恶劣环境鲁棒感知 + 诊断驱动融合兜底 + 闭环；所有模块收口成可讲解完整项目 | ✅ 完成 | `docs/P3_field_project.md` + `scripts/p3_flagship_delivery.py`（VGGT 托底实跑：自研VO塌缩 vs VGGT 0.02–0.03m，A\* 30 步/控制 29 条/避障 5 步） |
| **P4** | **真实驾驶动态建图（真实数据系统构建）** | 真实 RGB+真实 LiDAR 当实时输入流 → VGGT 定位 → LiDAR 定标 → BEV 增量建图 → 双视角回放 | ✅ 完成 | `docs/P4_real_driving.md` + `scripts/p4_real_driving.py`（KITTI 30 帧：轨迹 125.16m/定标 ×72.4/56.5 万真实点/双视角 GIF+MP4） |
...
### 2026-09-16（续63）· 🚗 新增 P5 真实驾驶端到端闭环 + P4 补全（硬件/参数/边界）
- **用户提问（关键）**："我们的 P4 是否全面了，硬件/算法的选择/能力/规则如何设置的？是否介绍清楚了一个完整的小型自动驾驶项目如何完整实现？"
- **盘点（先诊断）**：P4 把自动驾驶栈**前半段**（感知→定位→建图→可视化）做得很扎实且诚实，但 —— ① **完全没有规划/控制/决策/避障**；② **硬件是"匿名的"**（未说相机型号/线束/算力，与 `F7` 双向零引用）；③ **关键参数缺取值理由**（0.4m/80m/0.5m）；④ **§⑧ 诚实边界恰恰没承认"只做了一半"**；⑤ 项目里规划/控制在 P1/P2/P3/F6，但**全部基于仿真或室内手持数据，与 P4 的真实驾驶数据脱钩**。
- **用户选定方案**：A+B+C 补 P4 + 新增 P5 文档（用 KITTI 真实驾驶数据跑通感知→建图→规划→控制全链路）。
- **P4 补全（A+B+C）**：文件名 `docs/P4_real_driving.md`：
  - **A 诚实边界**：开头新增边界声明块（P4=前半段，指向 P5）；§⑧ 新增第 7 条"不含后半段（无规划/控制/避障，BEV 不能直接导航）" + 第 8 条"硬件匿名"。
  - **B 硬件对应（新 §⑨）**：KITTI 采集车 = PointGrey Flea2 相机（~1.4MP 卷帘快门）+ **Velodyne HDL-64E（64 线机械式，360°×27°，~10Hz，~120m）** + OXTS RT3000 GPS/IMU（demo 刻意不用）；§9.2 为什么选 KITTI（真实+相机LiDAR齐备+零下载+标定已做，代价是无位姿真值）；§9.3 算力需求（仅 VGGT 是算力大户 ≥16GB，其余纯 CPU）与端侧可行性。`F7` 同步补双向交叉引用。
  - **C 参数与规则设置（新 §⑩）**：§10.1 建图参数（BEV 0.4m/终图 0.3m、LiDAR 80m、着色 60m）；§10.2 移动目标检测参数（基础半径 1.2m、深度项 +0.03Z、cam_h 1.65m、离地阈值 cam_h-0.5=1.15m、累积策略）——**每个都给取值理由 + 调大调小会怎样**；§10.3 失效判据/降级规则表（VGGT 不可用/前向失败/有效像素<50/写入器打不开）+ **明确标注"移动目标误检目前无自动判据"** + 与 P3 量化判据（路径长<5%→兜底）的差距；§10.4 调参速查（哪些是 CLI、哪些写死需改代码）。
- **新增 P5**（`scripts/p5_real_driving_closed_loop.py` + `docs/P5_real_driving_closed_loop.md` + `experiments/P5_real_driving_closed_loop/`）：
  - **复用 P4 前端**：通过 `importlib` 动态加载 `scripts/p4_real_driving.py`，直接调 `load_drive`/`run_vggt`/`compute_scale`/`build_world_points`/`build_cam_points`/`detect_dynamic`——**保证"同一份真实数据、同一套前端"**，无重复实现。
  - **新增闭环五环**：① `build_occupancy_from_lidar`（世界点云→2.5D 占据栅格，**显式剔除移动目标栅格**，避免"地图幽灵"）；② `inflate_obstacles`（cv2.dilate 椭圆核，半径+余量）；③ `astar`（同 P3）；④ `diff_drive_control`（同 P3，含 a_max/ω_max 限幅）；⑤ `dynamic_avoid_from_detections`（用 P4 真实移动目标点做 VO 式前瞻减速）。
  - **实测数字（drive 0023，30 帧）**：占据栅格 487×369 @0.4m（≈195×148m）；障碍格 5038 / 可通 20570；**剔除移动栅格 8181**；**inflation 新增 5116 不可通行格**；A\* **492 步 / 211.84m**；控制 **491 条**（v_max 0.8m/s、ω_max 1.5rad/s）；**避让 34 步**；动态目标点 1743。
  - **产物**：`bev_occupancy.png` / `costmap_inflated.png`（膨胀前后对比，教学价值最高的图）/ `map_plan.png` / `control_cmd.png` / `dynamic_avoid.png` / `closed_loop.gif` + metrics.json + README（自动生成）。
  - **踩坑修复**：① `write_readme` 里中文引号 `"..."` 未转义导致 `SyntaxError: invalid character '→'`（改用 `「」`）；② `start_cell`/`goal_cell` 是 numpy int64 → `json.dump` 报 `TypeError`（显式 `int()` + `default=str` 兜底）。
  - **诚实边界（写进 P5 §⑧）**：自车运动是**运动学仿真**（非真车）；地图来自视觉估计无真值位姿（非评测型）；避障是**反应式 VO 简化版**（非 DWA/MPC，无轨迹预测）；**未做 ego-footprint clearing**（自车足迹清除——文档明确指出这正是"本 demo 与量产系统的差距"，也是规划路径贴走廊一侧走的原因）。
- **导航联动**：`00_INDEX.md`（覆盖表加 P5 行、路径 C/D 加 P5、图索引加 6 张 P5 图、覆盖状态新增 P5+P4补全说明）、`README.md`（文档地图加 P5 行 + 覆盖说明 + 成果一览新增"真实驾驶"图组）、`00_ROADMAP.md`（全局地图加 P5 + 最后一公里提示）、`P3_field_project.md`（诚实边界补"闭环输入是室内手持数据→见 P5"）、`F6_decision_control.md`（§9 对应实验表加 P5）、`F7_hardware_primer.md`（§10 补 P4 §⑨ 交叉引用）、本文件。
- **架构收获**：`P4 + P5` = 本仓库唯一一条"**真实驾驶数据上的 感知→建图→规划→控制 全链路**"。这也把项目的实战层从"各自独立的 P0–P4"升级为"P4/P5 成对构成一条真实驾驶完整栈"。

### 2026-09-17（续69）· 📊 P6 数据落地：真实数据集调研 + 可跑模拟脚本出图
- **动因**：用户要求「搜索一些数据或者模拟一些数据进行展示，加深认知」。
- **真实数据集 web-search（硬数字）**：MaSTr1325（1325 张、真 USV、两年跨度、语义分割：water/sky/obstacle 类）；SeaDronesSee（~3000 张、检测/SOT/MOT 跟踪、搜救场景、含 buoy/boat 类）；USVInland（首个内河多传感器 USV 集：SLAM/里程计+立体匹配+水体分割三任务）；MODD/MODD2（海事障碍物检测）；汇总源：Awesome-Waterway-Perception + MaCVi workshop。
- **新增 `scripts/p6_usv_sim.py`**（纯 numpy+matplotlib 零依赖下载，跑通）：合成内河航道 600×400@0.4m（岸/浅滩椭圆/桥墩/浮标/强反光区只覆盖水面）+ 2 艘动船在主航道 + 自实现 A\*（8 邻接欧氏启发、反光区代价 ×5）+ 膨胀 + 避障计数 + ATE vs 反光曲线。产出 `experiments/P6_usv_sim/`（2 图 + metrics.json 对齐 P5 格式）。
- **调试踩坑 3 个（诚实记录）**：① A\* 起点误放岸带内→无路径；② 懒删除判断误用 f(g+h) 对比 gsc(g)→跳过所有节点；③ `came` 以标量 `nr*W_+nc` 为键但判空用元组 `(gr,gc)`→永远判 not in 返回 None（BFS 验证可达后定位）。另有浅滩被 glare 覆写（改为 glare 只覆盖 FREE 格）+ CJK 缺字形（图内改英文标签）。
- **脚本自跑实测[模拟]**：栅格 600×400@0.4m，岸 48000/浅滩 1677/障碍 1608/反光 8323；A\* **561 步/240.6m**、**避障 7 步**、膨胀 54573 格、**路径经过反光格=0**（把反光区标高代价→规划层天然绕开感知退化区，物理上反光常在浅滩上方、脚本恰好耦合）；M6 兜底触发反光强度 ≈**0.55**。
- **联动**：P6 新增 §6.10（可复现脚本+产物表+认知点①②③）；PROGRESS 顶部 P6 状态补 §6.10。
- **认知收获**：真实数据集确认 P6 选型可行性（MaSTr1325 支持封闭集分割微调、SeaDronesSee 支持检测、USVInland 支持定位/水体分割三任务，各任务均有真值可回填）；模拟脚本把「假设数字」变成「可看图 + 可复跑代码」，是「加深认知」的最短路径。

### 2026-09-17（续68）· 🏗️ P6 工业证据附录：ODD/FMEA + 出海采集/标注 checklist
- **动因**：用户认可「开船出海采数只是工业流程一环」，要求补两份最划算的工业证据文档写进 P6 附录：(a) USV ODD + FMEA 安全概念模板；(b) 出海采集方案 + 真值载荷 + 标注规范实操 checklist。
- **新增 `docs/P6_new_scenario_perception.md` §7（ODD+FMEA 模板）**：ODD 表(地理/海况/时间/通视/交通/载荷)、安全概念 OSS(感知失效→降级→安全停船)、FMEA 抽样6条(强反光漂定位/GNSS丢星/来船漏检/浪花误检/浅滩/算力不足，每条含检测+缓解+严重度)、合规钩子(COLREGs 避碰规则 + DNV/ABS 船级社)。均为流程模板非实测。
- **新增 §8（出海采集 checklist）**：①出海方案(多工况航次/已测绘水域/PPS授时/RTK FIXED/MCAP原始流/边缘场景专采/回港校验)；②真值载荷清单(RTK双天线+光纤IMU 位姿/AIS+岸基 障碍/测深 水深/人工 语义，让"模拟ATE"变"真ATE")；③标注规范(对象/长尾主动学习/反光误标治理/时序一致性/双人仲裁IoU>0.85/回归集)；④与P6衔接诚实边界(流程模板非已执行，对外讲法)。
- **新增 §9 总收口**：P6 = 方法论(§2)+完整模拟(§6)+工业证据(§7/§8)，对应1–2年经验「知识×工程约束×生产级意识」。
- **联动**：PROGRESS 顶部 P6 状态补 §7/§8；呼应最初审视短板⑦⑧(系统设计交付物/团队评审证据)与 §1.1.1「感知为主控制为辅」。
- **下一步**：两份附录为模板；真实回填需出海/标注/认证(外部 gate)。其余待办见续67(④ C++ astar 等)。

### 2026-09-17（续67）· 🧪 P6 完整模拟演练（全部假设填充为可交付设计样例）
- **动因**：用户要求「把 P6 里所有的项目内容/条件/需求/算法/硬件约束都用假设的方式模拟出来」——即把方法论脚手架走成一份具体可讲的完整样例。
- **新增 `docs/P6_new_scenario_perception.md` §6**：全假设填充 = 场景/任务(内河港口巡检 USV, 500m 码头A→B, 微浪0.3m, 巡航1.5m/s)、硬件清单(IMX477全局快门相机+VLP-16 16线LiDAR+F9P RTK/IMU+单波束测深+Orin Nano本地GPU模拟)、控制契约(坐标/单位/目标精度数值)、子任务→算法选型(每项方法+关键参数+选型理由查F9+假设精度: VGGT开阔ATE0.18m/强反光0.85m、OWL零样本mAP0.58/微调0.84、分割mIoU0.91、A*350步等)、数据假设链(MODD下载成功/GNSS作伪真值/KITTI迁移兜底)、端到端管线(900帧/47.3m轨迹/12船8浮标3桥墩/2来船/600×400@0.4m栅格/350步A*/349控制/18避让/12%兜底)、模拟metrics.json、M8端侧预算(INT8 VGGT80ms+OWL90ms→4.8Hz, 功耗墙标代理)。每节明确标【模拟/假设】非实测。
- **诚实处理**：全节数字标【模拟】不得当项目实测引用（维护项目信誉）；控制为仿真、硬件为假设清单、端侧为本地GPU缩预算代理，与 §1.1.1/§4 一致。
- **联动**：PROGRESS 顶部 P6 状态补「§6 完整模拟演练」；§5 下一步加 ④ C++ astar 绑定回填。
- **下一步（真实回填）**：① MODD 真值换实测 mAP/ATE；② 改写 `p5_real_driving_closed_loop.py`→`p6_usv_perception.py`；③ 水域强反光 VGGT vs VO 失效对照；④（待确认）`astar.cpp`+CMake+测试。

### 2026-09-17（续66）· 🗺️ 新增 §1.1.1 整体策略定位 + P6 新场景从零起步方法论
- **动因**：用户要求（a）把 PROGRESS §1.1 定位句改成「**感知为主、控制为辅**」（一句话收口整体策略）；（b）新增 P6 论述「新场景什么都不具备时，怎么做感知给控制当输入」这一实际且易被考察的问题，以无人船自主驾驶为例。
- **§1.1.1 新增**（置于 §1.1 末尾）：明确整体策略=感知为主（主场：F9/M3/M5/M6/D1/P4/M5_industrial/M8/M7）、控制为辅（F6/P1/P5 仅接口打通+兜底设计闭环、运动学仿真级、不冒充真机），对外一句话「恶劣环境鲁棒感知方向，感知→控制闭环打通过（仿真级），真机控制待硬件，感知在真实公开数据定量跑通」。
- **新增 `docs/P6_new_scenario_perception.md`**（方法论补篇，非新算法）：6 步脚手架（控制契约反推→场景拆子任务→工具箱选型查F9→数据三路径→复用P5改写水域版→量化+诚实边界）；无人船贯穿例子含控制契约表/子任务映射/数据策略/水域特有坑(水面反光·无纹理·动态浪·尺度无GNSS·欠驱动)/`p6_usv_perception.py` 伪代码骨架/metrics 预案；诚实边界主动声明控制为仿真、硬件为零、水域真数据需外部下载、不冒充真机闭环。
- **联动**：顶部状态 + 本日志；P6 消费 F9/M3/M5/M6/M7/M8/P4/P5/M5_industrial，反哺「任意新场景通用适配方法论」。
- **下一步**：① 下载 MODD/USVInland 真值回填 metrics（受网络 gate）；② 把 `scripts/p5_real_driving_closed_loop.py` 改写为 `p6_usv_perception.py` 跑通最小闭环；③ 水域强反光区补「VGGT vs 传统 VO」失效对照（接 M5 方法动物园第 6 方法）。另待用户确认：写 `src/wildspatial/cpp/astar.{h,cpp}` + CMake + `tests/test_astar_cpp.py`（C++ 生产代码，呼应 P6 伪代码中的 `astar_cpp`）。

### 2026-09-17（续65）· 🏭 新增 M5 工业 SLAM 对照补篇（ORB-SLAM3 黑盒实跑）
- **动因**：用户要求"跑通一次工业 SLAM 当黑盒（ORB-SLAM3 或 LIO-SAM），写工程踩坑 + 与 VGGT 结论对照"，接 M5 方法动物园。同时用户确认整体定位：**重点在感知、弱化控制**（硬件不允许，感知对硬件需求更低）；端侧用本地算力小比例模拟即可（M8 缩预算代理成立，但简历里要讲清"代理≠真机"）。
- **新增 `docs/M5_industrial_slam.md`**（M5 方法动物园工业对照补篇）：
  - §0 目的（方法动物园缺产业级在线 SLAM 参照）；§1 选型（ORB-SLAM3 当黑盒，TUM fr1/desk 已在磁盘零下载；LIO-SAM 需 LiDAR 留待）；
  - §2 实跑步骤（依赖/构建/运行命令/associate.py/TUM fr1/desk + EuRoC 可选）；
  - §3 工程踩坑清单 7 条（Eigen 3.4 模板报错 / Pangolin 0.8 viewer API / headless 无 DISPLAY / 词表 txt 加载慢转 bin / TUM 需 association / 单目初始化需视差 / 单目尺度漂移）——**每条都回扣本项目 M1 坑#1/#3、M3、M5 发现**；
  - §4 对照表（自研VO 0.506 / ORB-SLAM3 0.0x / COLMAP 0.026 / VGGT 0.018·全档0.016–0.049 / 4Seasons夜间VGGT 0.415）+ 三条核心结论（ORB-SLAM3=工业级自研VO+回环v1；在线增量vs离线全局更本质呼应M5§4.2；单目尺度死穴+外部尺度源同源M6）+ 反哺（接入 `methods/` 并入5方法图谱 / 回环成功率当失效预警 / 有LiDAR补LIO-SAM）；
  - §5 诚实边界（实跑ATE为领域经验量级，未冒充项目实测，跑通后回填 `experiments/M5_orb_slam3/`）+ 与前后模块关系。
- **联动**：`docs/M5_stress_test.md` 新增 §9 指针（指 `M5_industrial_slam.md`，方法动物园扩到5方法对照）；本文件顶部状态 + 本日志。
- **下一步**：① 本机构建 ORB-SLAM3 跑 TUM fr1/desk（数据已就绪，受构建/网络约束属 gate）；② 接入 `methods/` 并入 M5 图谱；③ 有 LiDAR 真值流时补 LIO-SAM。

### 2026-09-17（续64）· 🔧 全库优化：补常识 / 深度 / 落地 / 试验（四维度）
- **动机**：用户"把这批更新推到 GitHub，然后继续补充优化项目——可以补常识、也可以补深度、补落地细节、也可以补试验"。
- **盘点（code-explorer 子代理：逐文档核对图片链接 + 脚本→实验→文档三段对应）**：整体成熟度高，缺口集中在 ① 破图（P3 7 张图缺 `../`）、② 选型/家族对比只讲单模型、③ 同步等工程坑只点到为止、④ 基础篇缺"初学者常漏"提示框。
- **补试验/可信**：
  - `docs/P3_field_project.md`：7 处 `](experiments/...)` 缺 `../`（在 `docs/` 下解析为 `docs/experiments/` 必破图）→ 全部改为 `](../experiments/...)`，`grep` 验证无残留。
  - `docs/F9_perception_methods.md`：原 354 行 0 图却"钉本项目实测"→ 在 §1/§4.1/§6.1/§8 嵌入 4 张实跑图（M5 失效图谱 / P4 BEV / M10 投影闭环 / M8 端侧权衡），让四联表数字有可视化佐证。
- **补深度**：
  - `docs/M4_foundation_models.md` 新增 §2.4「前馈 3D 基础模型家族」：VGGT/DUSt3R/MASt3R/CroCo 怎么选（输出/最强/最易崩/为何选 VGGT），诚实标注"仅 VGGT 实测，其余领域共识"。
  - `docs/M7_semantic_layer.md` 新增「开放词汇检测器三剑客」：OWL-ViT/Grounding DINO/YOLO-World 选型（最强/坑/何时选），呼应 §2.3 责任切分。
- **补落地**：
  - `docs/F9_perception_methods.md` §6.2 时间同步新增「同步实战排查清单」（点云飘/斜墙/慢半拍 → 先对钟→再查外参→最后怀疑算法），把"不同步最致命"落成可操作步骤。
- **补常识**：
  - `F0/F2/F3` 各加 `⚠️ 初学者常漏` callout：F0=坐标帧混用/Z 非斜距/标定漂移；F2=单目无深度/相对≠绝对/双目远处精度骤降；F3=退化下崩/忘回环漂/无全局尺度。
- **导航**：本次纯文档增强，无新增模块/路径，未改 INDEX/README/ROADMAP（保持导航稳定）。
- **GitHub**：P4 多场景+F9+P5 已 push（1866c8c）；续64 优化随后单独 commit 推送。

### 2026-09-16（续62）· 📚 新增 F9「传感器→场景重建：方法选型与工程清单」
- **用户需求（原话要点）**：回看项目，对算法的讲解是否足够——"如何从视觉和雷达等传感器数据中重建场景的算法：单目一般用什么方法、双目什么方法、目标检测用什么方法、准确性如何、雷达用什么方法、不同数据怎么处理、需要什么算力设备、训练要用多少数据多少类、没见过的物体怎么办"，以及"初学者想到的之外还有哪些要考虑"。
- **盘点结论（先诊断后动手）**：项目把"地基几何（M0/M1）+ 一个前沿模型（VGGT/M4）+ 压力测试（M5）+ 端侧（M8）"讲透了，但**"方法选型 + 数据/算力/长尾"这一层明显薄弱或缺失**——单目方法族选型对比、双目立体匹配/MVS、2D·3D 检测方法族与指标（mAP/NDS/mIoU）、LiDAR 配准（ICP/GICP/NDT）、训练数据量与类别体系、未见物体（OOD）、算力对照表，多数只有一两句或空白。
- **新增 `docs/F9_perception_methods.md`（354 行）**，风格按用户选定："每个方法族一张**四联表**（一句话定位 + 精度量级 + 算力·显存 + 需要的数据量），并钉本项目实测数字"。结构：
  - §0 全流程地图（5 工位 × 传感器）→ §1 单目（5 族 + 三个必踩坑：尺度漂移/纯旋转退化/快速运动）→ §2 双目/多视图（4 族 + 深度误差 ∝ Z² 直觉）→ §3 目标检测（2D 6 族 + 3D 5 族 + **类别数参考** COCO80/Cityscapes19/nuScenes23/KITTI3-8）→ §4 LiDAR（配准 5 族 + 检测分割 4 族）→ §5 场景表示（9 种 + 选型口诀）→ §6 融合（标定/时间同步/前中后架构）→ §7 训练（**数据量量级表 · 多少类 · 长尾 · 未见物体 OOD 五级应对**）→ §8 算力设备表（云端→车载→端侧→纯 CPU）→ §9 **初学者想不到但上真机就撞上**的清单（时钟同步/标定漂移/运动畸变/TF 树/标注成本/数据偏差/隐私合规/动态污染建图/回环/评测诚实/功能安全）→ §10 一页速查表 → §11 三条核心结论 → §12 与前后模块关系。
  - **钉入本项目实测**：M5 ATE 表（COLMAP 0.026 / VGGT 0.018 / 自研 VO 0.626 塌缩）、M10 KITTI（~18,500 点/帧，3.0–79.7m，重投影残差 0.0000px）、P4（定标 ×72.433，125.16m，56.5 万点，BEV ≈150×190m）、M7/D1（OWL-ViT 水下 recall≈0.001、夜间 ≈7 命中/图）、M8（INT8 PTQ）、M9（VO 9.8fps@CPU）。
- **导航联动**：`00_INDEX.md`（基础篇扩 F0–F9、覆盖表加 F9 行、路径 B/C/D 均加 F9、覆盖状态新增 F9 说明）、`README.md`（文档地图加 F9 行 + 覆盖说明更新为 F0–F9 + 新增 §6.6 选型速查段）、`00_ROADMAP.md`（全局地图集成层加 F9 + 最后一公里提示加 F9）、本文件（顶部状态 + 本日志）。
- **诚实边界（写进 F9）**："哪个方法好"没有绝对答案，只有"在什么约束下选谁"；除微调外绝大多数人不会从头训练（本项目 VGGT/OWL-ViT/YOLO 全用开源权重）；算力与精度量级为**领域经验量级**（非本项目实测的会明确标注"领域标准"），防止与项目实测数字混淆。

### 2026-09-16（续58）· 🚗 新增 P4 真实驾驶动态建图（真实 RGB + 真实 LiDAR 双视角回放）
- **需求**：用户提出"找真实数据集（自动驾驶街道/海上行船，RGB+雷达）当实时输入，车/船在其中开，构建当前场景，俯视全景+主体视角同时展示完成驾驶过程"。
- **选型**：海上无现成"RGB+LiDAR 行船建图"公开基准 → 选 **KITTI depth_completion**（已在磁盘 20G，真实车载 RGB + 真实稀疏 velodyne_raw + 稠密真值 + 内参，零下载）。
- **新增可跑脚本** `scripts/p4_real_driving.py`：① `load_drive` 按drive 抽连续片段当"实时输入流"；② 方法动物园 VGGT 一次前向出 30 帧 `T_cw`（up-to-scale）；③ `compute_scale` 用真实 LiDAR 深度 median 比定标（×72.433）；④ `build_world_points` 逐帧真实激光反投影→世界系；⑤ `render` 增量 BEV（0.4m 栅格高度着色，forward 朝上）+ 第一视角 RGB 叠真实扫描（近暖远蓝）+ 绿轨迹/红车体（最终分辨率绘制）→ MP4+GIF+关键帧；⑥ `render_final_map` 米制终图。
- **踩坑修复**：① `M[:3,3]` 广播错误（→`M[:3,3:4]`）；② 合成帧原生 ~12k px 超 MPEG-4 上限静默失败（→固定面板尺寸 400h，无需二次缩放）；③ 轨迹/车体在超大画布上绘制后被压缩到不可见（→先缩放 BEV 再在显示分辨率上绘制）；④ 终图轴刻度是像素不是米（→`imshow(extent=[米制])`）。
- **实测数字**：30 帧/drive 0023 ≈18s 行车；定标 ×72.433；轨迹 **125.16m**（≈7m/s 合理）；每帧真实激光 1.7–2.0 万点、累计 **56.5 万**；BEV ≈150×190m @0.4m。产出 `driving_dualview.mp4`（1946×400@3fps，半速播放）+ `driving_dualview.gif`（12.9M）+ 11 关键帧 + `bev_final_map.png` + metrics。
- **多场景 demo（同管线 4 条不同街道）**：`0023`(125.16m/56.5万点/1743 红) · `0002`(71.87m/22.6万/1187) · `0013`(152.62m/40.4万/**4771**) · `0095`(155.52m/55.3万/2281)；各产物独立成 `experiments/P4_real_driving/drives/<drive>/`（mp4+gif+11 帧+终图+metrics）。证明流程可迁移、非单街调参特例。
- **新增教程** `docs/P4_real_driving.md`：30 秒电梯演讲 + 场景/需求/真实问题 + 原理流程图 + 直白讲解 + 实测表 + 三视角讲解 + 初学者常漏 5 环节 + 复现命令 + 诚实边界（无逐帧真值位姿→不报 ATE；轨迹来自视觉、尺度来自激光；"雷达"=LiDAR；海上场景待数据集）。
- **导航联动**：`00_INDEX.md`（覆盖表+P4 行、路径 C/D 加 P4、图索引加 P4 2 图）、`README.md`（文档地图+状态表加 P4）、`00_ROADMAP.md`（全局地图链+P4 章节）、`DATASETS.md`（KITTI 条目补 P4 消费记录）、本文件（顶部状态+路线图表+本日志）。
- **验收**：脚本端到端 exit=0；双视角关键帧目检通过（左=真实 RGB+激光扫描、右=BEV+轨迹+车体）；MP4 ffprobe 校验 1946×400×30帧。

### 2026-09-16（续57）· 🚀 新增 P3 面试旗舰项目（理论→实战的收敛点）
- **用户诉求**：了解项目后，构建「能拿出去给面试官讲」的完整实战案例——有场景、有需求、有真实数据/框架、有端到端可跑代码、有量化结果、有工程叙事（从理论走向实战）。
- **动机**：项目纵轴（F/M 讲透单点）+ 横轴（P0 抽象管线/P1 工程叙事/P2 仿真）已闭环，但缺一个把「恶劣环境鲁棒性」这个王牌选题**收口成单个可讲解完整项目**的落点（PROGRESS §1.1「知识×工程约束=能力」的终局体现）。
- **新增可跑脚本** `scripts/p3_flagship_delivery.py`：
  - 在 TUM fr1/desk（带真值+深度）上施加合成退化（弱光 0.9 / 运动模糊 0.7），跑方法动物园（自研VO/COLMAP/VGGT/LightGlue）；
  - **实测结论（100 帧）**：自研 VO 在弱光/运动模糊下**路径长=0（塌缩、冻结原点）**，VGGT 三档 ATE **0.020/0.022/0.030m 稳压**、COLMAP 0.028/0.067/0.026、LightGlue 0.49/0.30/0.42；
  - **诊断驱动融合兜底**：主方法塌缩→切 VGGT 托底；托底轨迹定尺度后驱动**占据栅格+A\*（30 步）+差速控制（29 条，v_max 0.30/ω_max 1.2）+速度障碍法避障（5 步）**完整闭环；
  - 产出 4 张图（robustness_contrast/map_plan/control_cmd/dynamic_avoid）+ metrics.json + README。
- **共享代码改动**：`methods/vggt.py`/`methods/colmap.py` 新增完整 `T_cw` 位姿输出（`extra["T_cw"]`），供闭环建图/占据栅格复用（此前仅返回相机光心 positions）。
- **新增教程** `docs/P3_field_project.md`：30 秒电梯演讲 + 场景/需求/真实问题 + 专业原理 + 直白讲解 + 系统架构图 + 真实验证（引 P3 4 图 + 4Seasons 真实夜间轨迹 + M5 图谱 + M8 三维表）+ 三视角讲解 + 初学者常漏 10 环节 + 复现命令 + 诚实边界。把 F0–F8/M0–M9/P0–P2 全部收口。
- **导航联动**：`00_INDEX.md`（覆盖表+P3 行、路径 C/D 加 P3、图索引加 P3 4 图 + 4Seasons 夜间轨迹）、`README.md`（文档地图+状态表加 P3）、本文件（顶部状态+路线图表 P3 行+本日志）。
- **验收**：脚本端到端 exit=0，4 图均生成；全仓库图片引用新增 4 处（P3）+1 处（4Seasons 夜间轨迹），均指向已生成文件。

### 2026-09-15（续52）· 🖼️ 教程打磨：F7 硬件扫盲补"实际效果展示 + 真实场景串联"
- **用户反馈**：继续完善教程，要求"多一些实际效果展示 和实际场景的实际例子的结合"。
- **F7 新增/增强**：
  - §3 摄像头补"实际效果展示"：仿真 RGB-D 输出图(gazebo_rgbd) + 真实夜间(night_det)/水下(underwater_det)场景图（印证 §3.2 HDR/快门为何重要）。
  - §4 激光雷达补"实际效果展示"：KITTI 真实稀疏扫描线点云(kitti_lidar_fusion) + LiDAR→costmap 地图(costmap_concept)。
  - §10 补动态避障真实效果(ped_tracking：行人距离 vs 避障指令)。
  - **新增 §13 实际场景串联**：一趟"园区夜间配送"任务，用表格逐时刻拆解 6 类硬件谁在工作/采什么数据/谁兜底，并配 3 张真实数据图(ped_tracking/sensor_fusion/night_det)印证——把硬件冗余与 F6 三层兜底(§3.7.4)打通。
- **索引联动**：`00_INDEX.md` 图索引给 6 张图(gazebo_rgbd/costmap_concept/sensor_fusion/ped_tracking/night_det/underwater_det)补 F7 标签。
- 教程从"清单罗列"升级为"看得见 + 真实场景串联"，更贴合初学者建立直觉。

### 2026-09-15（续53）· 🎬 教程完善：新增「F8 全栈贯通」把碎片拼成一条线（一镜到底）
- **用户反馈**：继续完善项目，秉持"讲清楚讲透彻 + 多一些实际效果展示 + 和实际场景结合"的原则。
- **动机**：F0–F7 + M0–M9 是知识点碎片，但真实机器人每秒同时调用它们；缺一篇"把碎片串成一条线"的贯通叙事。
- **新增 `docs/F8_real_scenario_walkthrough.md`**：用同一个真实任务「园区夜间配送」拆成 9 个时刻（标定→走廊定位→融合感知→填 WorldModel→costmap+A\*→动态避障→差速+PID 控轮子→三层兜底→门口送达），每时刻讲清"谁在工作(硬件)/算什么(算法,引 F/M)/吐什么数据/出事谁兜"，并嵌入**本项目已实跑的 ~19 张真实图**（keyframes/sensor_fusion/VO 轨迹/open_vocab/KITTI LiDAR/投影闭环/VGGT 深度/夜间·雾·反光退化图/costmap/A\* 规划/NavFn vs 手搓/动态避障/控制指令序列等）；附"全链路一图流对照表"和"从一个场景看懂整个教程"收尾。与 `F7 §13`（硬件器官视角看同一趟任务）两面互补。
- **织入导航**：`00_INDEX.md`(基础篇扩 F0–F8、覆盖表加 F8 行、路径 B 加 F8、路径 D 扩 F0–F8、覆盖状态更新、19 张图补 F8 标签)、`00_PRIMER.md`(模块地图加 F8 行、补 §2.10 全栈一张图、学习路线加步骤 9)、`00_ROADMAP.md`(全局地图集成层加 F8、新增 F8 模块、最后一公里提示加 F8)、`README.md`(文档地图+目录树+覆盖说明+快速路径更新 F0–F8)；`F6`/`F7` 加指向 F8 的交叉引用。
- 教程从"各篇孤立"升级为"全栈一张图"——所有篇章都能在同一个真实场景里被"看见"，初学者回看任何一篇都不再孤立。

### 2026-09-15（续54）· 🎯 教程优化：P2_simulation.md 理论段补真实场景实例（防光讲理论）
- **用户反馈**：继续完善优化，多结合实际场景和落地例子，防止光讲理论。
- **诊断**：`P2_simulation.md` 前 1/3（§1 目的、§2 平台对比、§3.5 世界表示谱系、§3.6 同源模型、§3.7 感知-决策解耦）偏"空讲概念/表格"，缺把抽象落到同一真实场景的实例；§4 实验段已很实，未动。
- **改动（6 处补丁，均落到「园区配送/夜间送货」同一真实场景 + 引本项目真实图）**：
  1. §1 加"园区送货机器人"实例（回放 vs 仿真冲人，呼应 §4.14）；
  2. §2.1 加"场景→平台"选型决策清单（扫地/自驾/机械臂/VLN 对号入座）；
  3. §3.5.3 补"costmap 怎么长出来"实例（static/obstacle/inflation 三层各填什么，引 costmap_concept.png）；
  4. §3.6.3 补"用同源给 VGGT 打分"实例（引 §4.9 RMSE≈0.17m）；
  5. §3.7.4 补"夜间漏检行人→三层兜底接住"翻车案例（时间线表 + 引 degradation_gallery/ped_tracking）；
  6. §3.8 新增"三节串成一个场景"收口段，指向 F8 全栈贯通。
- 现在 P2 从"平台/理论清单"升级为"概念全部能在园区配送真实任务里被看见"，与 F8 互文。

### 2026-09-15（续55）· 🖼️ 数据集实跑：补全 DATASETS.md 真实样例图 + 尝试 4Seasons 真实数据 ATE
- **用户反馈**：继续完善，多跑一些下载数据集的结果 / 用搭好的仿真平台，仅供参考。
- **做法 1（已落地，真实缺口修复）**：`experiments/datasets_showcase/figs/` 此前为空，而 `docs/DATASETS.md` 已引用 5 张图（nightcity/sunrgbd/ndispark/modelnet40c/fourseasons）→ 断链。用 `scripts/datasets_showcase.py` 从已下载 ModelScope 数据抽样，生成全部 6 张真实样例图；并修掉 underwater 因图像在 train/valid 子目录被跳过的问题（递归扫描），新增 `underwater.png`；在 DATASETS.md §9 补水下真实样例图引用。
- **做法 2（受阻，未写进文档）**：尝试 `scripts/run_4seasons.py` 用真实夜间驾驶数据做方法动物园 ATE 对比。结果：(a) GPU 被另一进程占 20GB（仅剩 3.4G），VGGT/LightGlue 显存 OOM；(b) `handcrafted_vo` 反复返回 ATE≈1e-16（疑似 GT 采样过粗致对齐退化 或 方法/对齐 bug），`colmap` 未注册不可用 → 结果误导/不可用，未引用。已留 `experiments/M5_4seasons_oldtown_night/` 半成品（用户保留未删）。需 GPU 空闲 + 排查 handcrafted_vo ATE≈0 后才能作为干净结果。
- **结论**：本轮最扎实产出是"6 张真实数据集样例图"——把 DATASETS.md 从断链修复为"每个数据集都看得见真实样本"，直接回应"多跑/展示下载数据结果"。

### 2026-09-16（续56）· 🌙 4Seasons 真实夜间 ATE 评测跑通（修 3 个 bug + 4 方法对比 + M5 §4.2）
- **用户反馈**：删除不可信的 4Seasons 半成品评测产物；GPU 已空闲，继续相应操作。
- **已删** `experiments/M5_4seasons_oldtown_night/` 半成品，然后正式重跑成功。
- **排查并修复 3 个 bug（此前 ATE≈1e-16 不可信的根因链）**：
  1. **时间戳解析 bug（致命）**：4Seasons 图像文件名本身即纳秒时间戳，但 `times.txt` 在 `recording_.../times.txt`（不在 `cam0/` 子目录），脚本按 `cam0/times.txt` 找 → 找不到 → `stamps` 全 0 → `nearest_pos` 把所有帧对到**同一个 GT 点** → GT 退化成常数 → **所有方法 ATE 全部失真**（这才是 ATE≈1e-16 的真因，不是 handcrafted_vo 的锅）。修复：直接用图像文件名当时间戳（与 GNSSPoses 同尺度）。
  2. **退化判定误伤 VGGT**：先按"原始轨迹路径长 < GT 5% 判退化"，但单目输出尺度任意（VGGT raw 0.31m 实为未对齐），Sim3 对齐后形状完全正确 → 误判。修复：改用 **Sim3 对齐后**的路径长判定。
  3. **脚本内相对导入崩溃**：`from ..eval.trajectory import ...` 在顶层脚本里 `..` 失效 → 全方法标"异常"。修复：改绝对导入。
- **追加修复（诚实性）**：(a) COLMAP 返回的 `frame_indices` 是其自身注册顺序（乱序），路径长/轨迹图必须按帧时间排序（Umeyama 对序无关，ATE 本就正确）；(b) 给"跟踪丢失"单独标记（est_len<5% GT），不报误导性 ATE；(c) CLI 方法名 `colmap` → `colmap_sfm`；(d) 新增俯视轨迹对比图 `traj_<method>.png`（VGGT/COLMAP 轨迹紧贴 GNSS 真值）。
- **最终结果（40 帧真实夜间切片，图像亮度均值仅 3/255，SIFT 每帧仅 ~130 特征）**：
  | 方法 | 路线 | ATE(m) | 对齐路径长/GT | 状态 |
  |---|---|---|---|---|
  | VGGT | 前馈基础模型（离线全局） | **0.415** | 6.52/6.75 | ✅ |
  | COLMAP | 工业级 SfM（离线全局） | **0.444** | 6.58/6.75 | ✅ |
  | 自研 VO（SIFT） | 在线增量 | — | 0.00/6.75 | ❌ 冻结在原点 |
  | LightGlue VO | 在线增量（学习前端） | — | 0.00/6.75 | ❌ 冻结在原点 |
- **核心洞察**："离线全局 vs 在线增量"的分野比"经典 vs 学习"更本质——增量 VO 靠帧间跟踪续命，夜间可重复特征太少一断即死；全局方法联合求解，单帧崩了能被拉回。且真实夜间 VGGT 0.415m vs 合成低光 0.019m（差 ~20 倍）——实证 §2.3"合成≠真实"，但方法排序保住。
- **文档落地**：`M5_stress_test.md` 新增 **§4.2 真实退化验证**（表格+traj_vggt.png+复现命令+工程含义，直接回应 §2.3 诚实声明）；`DATASETS.md` §2 4Seasons 条目补实跑结果链接；产出 `experiments/M5_4seasons_oldtown_night/`（metrics.json + ate_bar.png + 4 张 traj 图 + README.md）。

### 2026-09-15（续51）· 🔧 教程完善：新增「F7 硬件扫盲」补齐硬件认知（算法人必懂的器官清单）
- **动机**：用户指出教程只讲相机数学(F0)/融合概念(F5)，缺"硬件扫盲"——摄像头买哪种/看哪些参数/采什么数据、激光雷达与毫米波雷达区别、机器人靠什么动、同步标定等。
- **新增 `docs/F7_hardware_primer.md`**：四类器官框架（感知输入/计算平台/控制输出/神经能源）；逐器件讲"长啥样·测什么·关键参数·吐什么数据"——摄像头(单目/双目/RGB-D/鱼眼/事件 + 选型口诀 分辨·帧率·快门·焦距·接口)、激光雷达(机械/固态 + 线束/测距/点频/FOV + 点云 x,y,z,intensity)、毫米波雷达(多普勒测速)、深度相机(ToF/结构光/主动双目)、IMU/编码器/GPS/超声波、电机(直流/无刷/步进/舵机)/驱动器/运动机构、MCU/Jetson/总线/同步标定；附"园区配送机器人选型清单 6 类硬件"；引用 P2 多传感器同步/KITTI 真实 LiDAR/仿真 RGB-D 图。
- **织入导航**：`00_INDEX.md`(基础篇扩 F0–F7、覆盖表加 F7 行、路径 B 加 F7、路径 D 扩 F0–F7、覆盖状态更新)、`00_PRIMER.md`(补 §2.8 感知到控制 + §2.9 硬件器官 正文[此前 §2.8 仅留引用未写入]、模块地图加 F7 行、学习路线加步骤 8)、`00_ROADMAP.md`(全局地图基础层标 F7、新增 F7 模块、最后一公里提示加 F7)、`README.md`(文档地图+目录树+覆盖说明+快速路径更新 F0–F7)。
- 与 F0(相机数学)/F5(融合概念)形成"硬件侧 vs 数学/融合侧"互补；教程从"纯感知算法"升级为"硬件→感知→控制 全栈"。

### 2026-09-15（续50）· 📚 教程完善：新增「F6 决策与控制」补齐感知控制全栈
- **动机**：现有教程 F0–F5 + M0–M9 偏"感知"，"控制/决策 + 感知-控制集成"只在 P1/P2 零散覆盖；用户目标"培养初学者成为感知控制专业人士"。
- **新增 `docs/F6_decision_control.md`**：初学者友好讲 WorldModel 接口(衔接 §3.7)→占据栅格/代价地图→全局规划(A*/NavFn)→局部避障(速度障碍法)→差速轮+PID→三层兜底；引用 P1/P2 真实跑通图（costmap/map_plan/navfn_vs_p1/dynamic_avoid/ped_tracking/control_cmd）。
- **织入导航**：`00_INDEX.md`（基础篇扩为 F0–F6、覆盖表加 F6 行、路径 B 加 F6、新增路径 D「感知控制专业人士」、图索引标 F6、覆盖状态更新）；`00_ROADMAP.md`（全局地图加集成闭环层、新增 F6 模块、优先级补"最后一公里"）；`00_PRIMER.md`（新增 §2.8 感知到控制、模块地图加 F6/P1/P2、学习路线加步骤 7）；`README.md` 文档地图+目录树+覆盖说明更新。
- 与 §3.7 架构认知（续48/49 写入）形成"教学版 vs 架构版"两面，共同把教程从"纯感知"升级为"感知控制全栈"。

### 2026-09-15（续49）· 🧠 P2 架构认知：感知-决策解耦 + 兜底分层（写入文档）
- **用户洞察**：难点在现实场景感知；可分两阶段——建模好→退化为走格子（同仿真），建模差→需兜底。
- **结论（已记录 `docs/P2_simulation.md §3.7`）**：
  - Stage 1（感知建模，M7）把现实→结构化世界模型；Stage 2（决策控制，M4/M5/M6）只消费该抽象接口，与来源无关。
  - 建模好 ⇒ Stage 2 输入与仿真同构 ⇒ 退化为几何/走格子问题（FPV demo 用真值即此情形）。
  - 建模差时的三层兜底：① 不确定性感知（costmap 带置信度）、② 反应式安全层（读原始传感、跨过 Stage1 语义）、③ 降级策略（置信度低→减速/停车/请求人工）。
  - 待做建议：把 Stage 2 写成 `WorldModel` 接口（障碍+栅格+置信度），M7 与真值填同一接口，加反应式兜底量化"真值 vs M7"差距。
- 联动：`docs/P2_simulation.md` 新增 §3.7（核心认知），与 §3.5 世界表示谱系、§3.6 解构方法论、§4.7 感知实验台、§4.15/§4.16 FPV demo 交叉引用。

### 2026-09-15（续48）· 🎥 P2 新增「Gazebo 真实 RGB 相机 FPV」（方案 A 落地）
- **触发**：用户执行 `sudo usermod -aG render hmn-cjy`，`/dev/dri/renderD128` 权限问题解决。
- **先修探测脚本 `scripts/_probe_fpv.py`**：把 `robot_sdf=` 改为 `robot_sdf:=`（ros2 launch 参数分隔符 bug），
  然后用 `sg render` 重跑，确认 RGB/Depth 都收到真实画面（RGB mean=180, std=50，非灰屏）。
- **新增 `scripts/p2_delivery_fpv_gazebo.py`**：
  - 复用 `open_plaza_dynamic.sdf.xacro` 广场世界，TB3 从 `(0,-3.8)` 前往 `(0,3.8)`；
  - 订阅 `/clock` 用**仿真时间**积分（本机 headless 负载高，仿真时间慢于墙上时间），保证 HUD/小地图与真实相机画面同步；
  - 行人位置按世界 xacro 的 `VelocityControl` 参数解析计算；
  - 触发 ①静态障碍绕行、②动态行人停车让行、⑥送达点到达；
  - 输出 483 帧 GIF（3.4MB）+ MP4 + 关键帧 + metrics.json。
- **新增 `experiments/P2_delivery_fpv_gazebo/README.md`**。
- **联动**：`docs/P2_simulation.md` 新增 §4.16；`docs/00_INDEX.md` 图索引新增 Gazebo 真实相机图；`README.md` 文档地图更新；本文件续48。

### 2026-09-15（续47）· 🎬 P2 新增「机器人第一视角 3D 送货演示」（用户要求的复杂版本）
- **用户诉求**：保留当前俯视基础版，再做一个"复杂情况"的——**机器人视角、3D 画面、去固定地点送货、路上出现各种情况**。
- **关键阻塞**：Gazebo `--headless-rendering` 需要 `/dev/dri/renderD128`，当前用户**无 `render` 组权限**，
  RGB 相机输出灰屏（`scripts/_probe_fpv.py` 实测确认）。
- **方案选择**：先走 **方案 B（纯合成 3D FPV，零 GPU 权限依赖、可立即运行）**，方案 A（`usermod -aG render`）作为后续升级选项。
- **新增 `scripts/p2_delivery_fpv.py`**：
  - 自研针孔相机投影 + 透视变换 + **画家算法**（远→近排序覆盖），CPU 实时渲染机器人眼中的 3D 世界；
  - 机器人沿固定路线前往目标建筑 `(0,10)`，途中依次处理 **6 类路况**：
    ①静态障碍绕行、②横向行人停车让行、③对向行人靠右礼让、④视觉盲区拐角减速、⑤窄通道居中通行、⑥送达点到达。
  - 输出：GIF 2.6MB（218 帧@12fps，约 18s）、MP4 0.5MB、关键帧拼接图、逐帧 `metrics.json`。
- **新增 `experiments/P2_delivery_fpv/README.md`**（四段式：目的/6 类路况/实现/边界）。
- **联动**：`docs/P2_simulation.md` 新增 §4.15（含实现方案、关键帧、动图、与俯视版对比）；`README.md` 文档地图更新；本文件续47。

### 2026-09-14（续46）· 🔴 修复「避让不够优」——关键教训：**先修场景，再修算法**
- **用户反馈**："行人进入其黄色虚线范围后的避让不够优呀"。
- **先调了三版算法（都没解决）**：v1 开关式（进圈固定 off/v，贴脸仍全速：<0.7m 时 v 恒 0.15，133/133 帧）
  → v2 比例式（速度/偏转随距离连续）→ v3 预测式（未来位置触发 + 停车侧移）。
  最小距离始终 **0.29~0.38m < 碰撞距离 0.44m**（均实际穿透）。
- **v2 关键发现**：dmin 最低帧处机器人**已静止（v=0 持续 12 帧），dmin 却从 0.41→0.29** ——
  是**行人在主动走向静止的机器人** → **"停车"是被动策略**，避不开被撞。
- **🔴 真因（几何分析）**：机器人环线 `x²+y²=1.5²`；行人路径 x=0 交于 y=±1.50、x=1.2 交于 y=±0.90
  → **4 个必经冲突点**，机器人必须穿过人流才能巡逻，**算法再好也避不开** —— 我此前在**错误的地方使劲**。
- **修复（场景优先）**：行人从 x=0/1.2（纵切环线中央）移到 **x=±2.6（环线外侧）** → 冲突点 4→**0**，机器人可绕到行人身后。
- **最终实测（v4）**：最小距离 **0.293→0.782m**（>0.44m，余量 1.8 倍 ✅）；停止圈 **0.0%**；安全巡航 **90.1%**；
  **速度-距离严格单调 0.130→0.197→0.263→0.278**（教科书式比例控制）。
- **保留的算法改进**：比例减速 `v=v_max(d−STOP)/(WARN−STOP)` + 比例转向（5°~75°）+ 速度预测前瞻 0.8s + **分层安全区可视化**（黄1.2/橙0.85/红0.5m）。
- **🔑 核心教训（可复用）**：**"避障不够优"未必是算法问题，先检查场景是否几何可避**；
  诊断手段 = **把机器人路径与障碍路径画在同一坐标系求交点**，交点在必经区 = 场景问题。顺序：**① 几何可避 → ② 算法优化**。
- 联动：`docs/P2_simulation.md §4.14.3b` 新增「修复四」（四版对比表 + 几何分析 + 教训）；`open_plaza_dynamic.sdf.xacro` 行人站位修正；本文件续46。

### 2026-09-14（续45）· 🔴 修复动图「目标点形同虚设」（用户追问驱动，范式性错误）
- **用户追问**："这里机器人的目的是什么，没有和五角星目标有什么关系呀，没有朝着五角星前进"——**完全正确**。
- **代码级根因**：旧版控制律用**环线解析切线**（`atan2(nxt-cur)`，来自时间函数），**全程未使用 goal**；
  而 `goal=patrol_target(now+1)` 只是同一时间函数提前量，与机器人位置**无因果闭环** →
  机器人不朝紫星走、紫星自己转圈、两者独立，目标点沦为装饰。
- **本质错误**：混淆两个范式 —— **轨迹跟踪**（跟切线+横向误差）vs **目标点导航**（`atan2(goal-robot)`）。
  旧版写的是前者，却画了后者的目标点。
- **修复（标准航点追逐）**：① 固定航点表（八边形 8 点），紫星=当前追逐航点；
  ② 朝向 = `atan2(goal-robot)` **直接朝目标**；③ 距目标<0.28m → **切换下一航点**；
  ④ 避让在"朝目标"上叠偏置；⑤ 图上新增**机器人→目标连线 + 朝向箭头 + 到达圈**，HUD 加**到目标距离 + 朝向误差**。
- **实测验证**：**朝向误差中位数 0.1°**（巡航几乎完美对准）；<15° 占比 56.9%；
  均值 26.3° 被**避让帧**拉高（`off=±1.1rad=63°`，自洽）；到目标距离**周期收敛 0.28m 后跳走**；**目标点跳变 11 次**（=11 次到达切换）。
- **深层价值**：**"能画目标点"≠"真的朝目标走"**。只看截图/只看跑通会掩盖此范式错误，
  是用户"它到底在干什么"的**行为层质疑**才暴露它 —— 与续44（动图暴露卡死）同一方法论：**可视化+质疑行为才能发现逻辑错误**。
- 联动：`docs/P2_simulation.md §4.14.3b` 新增「修复三」（含范式对比表 + 实测验证表）；本文件续45。

### 2026-09-14（续44）· 🐛 修复动图「行人跳来跳去」（用户反馈驱动，含实测根因）
- **用户反馈**："现在行人咋跳来跳去的？目标点在动是正常的吗，在追踪吗？"
- **实测坐实（比预想更严重）**：探针脚本 1s 采样行人位姿，发现
  - t=2.0s 行人 y 从 **4.50 突跳到 1.78（Δ=2.7m）**——**撞墙穿透（tunnel）后弹出**；
  - t=5s~13s 恒为 y=4.68（=5.0−0.5−0.22），**卡死贴在北墙**。
- **根因**：世界行人只被赋予**恒速** `initial_linear=0 ped_speed 0`，**无边界/折返逻辑**；
  `mu=0`（零摩擦）+ `VelocityControl` 持续施力 → 滑到 10×10m 外墙 → 穿透瞬移 → 卡死。
- **修复**（`scripts/p2_dynamic_obstacle_gif.py`）：脚本对行人做**边界反射**——
  按真值位置监控，`|y|>3.0` 即向 `/model/pedestrian{1,2}/cmd_vel` 发**反向速度**，
  使行人 y∈±3.0m 平滑往复（离墙留 1.5m 余量，从根上不碰墙）。
- **实测修复对比**：y 范围 −4.68~4.68 → **±3.0**；越界次数 **→0**；单步位移 2.7m 瞬移 → **平滑 ~0.35m/s**；卡死 → **持续往复**。
- **顺带澄清**：「目标点在动」**不是追踪**，是环线上前瞻 1s 的航点（时间函数生成，与机器人无关）；
  新增**环线虚线** + 行人**运动方向箭头**，让语义在图上直接可读。
- **顺带提升**：采集 5Hz→**12Hz**、GIF 6→**10fps**（495 帧 / 2.8MB）；修正 `patrol_target` 注释错（写 2.0m 实为 1.5m）；
  修掉自己引入的 `%` 格式化占位符数量不匹配 bug。
- **关键坑记录**：`VelocityControl` 指令 topic 为 `/model/<name>/cmd_vel`（实测 `gz topic -l` 确认）。
- **联动**：`docs/P2_simulation.md §4.14.3b`（重写，含实测表）+ §4.14.5（新增 3 条踩坑）；本文件续44。
